...
/* 2026-08修正：LPBN_APP/PC原本被歸類在「沒有檔案大小限制→PNG無損」那組，
   ...
   2026-08再新增：ar也改成JPG輸出（使用者明確要求），一樣沒有K數上限、
   固定高畫質，不用試壓縮。 */
var EXPORT_JPEG_NO_LIMIT = { '11_lpbn_app': true, '12_lpbn_pc': true, '04_ig': true, 'ar': true };
          ...
          applyDefaultLogos(function(){
            ...
            var jobs = order.map(function(layout, idx){
              ...
              return exportLayoutBlob(layout.id, canvas).then(function(res){
                /* 2026-08修正：使用者反映不需要每個頁籤各自一個子資料夾，
                   全部檔案直接放在zip最上層、跟暫存檔同一層就好。原本用
                   zip.folder(folderName)分子資料夾，改成直接zip.file()放
                   在根目錄。使用者確認這個專案(電子票券繳費)整包zip固定
                   只會有一個頁籤，不會有跨頁籤撞名的疑慮，檔名不用加
                   folderName前綴，維持單純的「序號_版位名稱.副檔名」。 */
                zip.file((idx+1)+'_'+sanitizeFilename(layout.name)+'.'+res.ext, res.blob);
              });
            });
