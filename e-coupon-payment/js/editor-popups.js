...
function runImport(excelFile, folderFiles){
  ...
  function processOneBlock(parsed, done){
    ...
    /* 券樣合成（流程第6a步）：只有這個區塊「曝品」表裡真的有槽位標記
       票券化(見editor-import.js的TICKET_SLOT_TO_PRODUCT_SLOT)才會跳出來，
       其餘工單完全不受影響、也不會多跳一個空的popup。 */
    function goToShadowStep(){
      renderAll();
      var ticketSlotIds = (exposure && exposure.items ? exposure.items : [])
        .filter(function(it){ return it.ticket; })
        .map(function(it){ return it.slot; });
      if(ticketSlotIds.length && typeof openCouponTicketPopup === 'function'){
        openCouponTicketPopup(ticketSlotIds, matched, function(){
          renderAll();
...
function proceedToShadowFromImport(matched, exposure, onConfirm){
  ...
  var polaroidSlotIds = []; // 「拍立得呈現」欄=True的槽位，全部讀完之後統一排隊自動跳出調整popup
  matchedShadowKeys.forEach(function(slotId){
    ...
    /* isTicket：Excel曝品表這格填的是「票券N(左/中/右)」而不是「商品N(左/中/右)」
       （見editor-import.js的TICKET_SLOT_TO_PRODUCT_SLOT）——代表這格這次要走
       券樣合成(LOGO疊在固定PNG券框上)，不是放一般商品照片。目前券樣合成popup
       (流程說明文件第6a步)還沒做，這裡先把旗標存起來，資料不會遺失，等popup
       接上時直接讀S.shadowSlots[slotId].isTicket決定要不要跳出來、疊哪張框。 */
    var isTicket = !!matched[slotId].__importIsTicket;
    /* 使用者反映：券樣不要吃Excel「商品比例」欄位的大/中/小縮放，固定用
       同一個尺寸（PRODUCT_POS_LEFT_TICKET.hPct那組固定值，見
       shadow-layout-defaults-circle.js）——不管Excel這格填大/中/小，
       券樣一律強制ratio=1，忽略Excel給的倍率，不像一般商品那樣照實際
       填的值縮放。 */
    if(isTicket) ratio = 1;
    /* ticketMeta：如果這個File是coupon-ticket-popup合成出來的(見openCouponTicketPopup)，
       上面會掛著這次用的LOGO/顏色/文字，存進S.shadowSlots讓「票券編輯」按鈕
       可以還原上次的設定，不用每次都從零開始（見shadow-popup.js的
       「票券編輯」按鈕／coupon-ticket-popup.js的openCouponTicketEditForSlot）。 */
    var ticketMeta = matched[slotId].__ticketMeta || null;
    if(matched[slotId].__importPolaroid) polaroidSlotIds.push(slotId);
    var reader = new FileReader();
    reader.onload = function(ev){
      S.shadowSlots = S.shadowSlots || {};
      S.shadowSlots[slotId] = { dataUrl: ev.target.result, type: type, ratio: ratio, isTicket: isTicket, ticketMeta: ticketMeta };
      pending--;
      if(pending<=0){
        S.shadowCombo = comboFromExcel || guessComboFromMatchedSlots(matchedShadowKeys);
        openShadowPopup(onConfirm);
        if(polaroidSlotIds.length) autoOpenPolaroidQueue(polaroidSlotIds);
      }
    };
...
/* 「拍立得呈現」欄=True的槽位，匯入完打開1200畫布之後，自動依序跳出
   ShadowFramePlugin的拍立得調整popup，讓使用者確認/調整構圖——不用像
   以前那樣，即使Excel已經勾了，還要自己找到那個槽位手動點「拍立得」
   checkbox才會生效。多個槽位的話排隊依序跳出，一次只開一個，使用者
   確認完/關掉這個，才會接著跳下一個，不會同時疊好幾個popup在畫面上。 */
function autoOpenPolaroidQueue(slotIds){
  var queue = slotIds.slice();
  function step(){
    var id = queue.shift();
    if(!id) return;
    if(typeof togglePolaroid === 'function'){
      togglePolaroid(id, true, step);
    } else {
      step();
    }
  }
