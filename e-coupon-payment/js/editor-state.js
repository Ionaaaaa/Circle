...
/* 2026-08新增：公版版本從A/B兩組擴充成A~H八組（見configs/theme.json、
   backgrounds/{A..H}/、logos/{A..H}/）——這支函式統一負責「把任意輸入值
   轉成合法版本代號」，任何地方要用S.templateVersion/d.templateVersion之前
   都先過這支函式，不合法(不是A~H其中一個、或undefined/null)一律退回'A'。
   之前A/B兩版時代，程式裡到處寫死「=== 'B' ? 'B' : 'A'」這種二選一判斷，
   擴充成8組時這些地方全部要跟著改，容易漏改；改成集中一支函式之後，以後
   如果還要再擴充(例如變成A~J)，只要改這裡的TEMPLATE_VERSIONS陣列一個
   地方就好。 */
var TEMPLATE_VERSIONS = ['A','B','C','D','E','F','G','H'];
function normalizeTemplateVersion(v){
  return (typeof v === 'string' && TEMPLATE_VERSIONS.indexOf(v) !== -1) ? v : 'A';
}
...
/* 2026-08新增：'05_ddcard_nologo'是「真正獨立」登記在LAYOUT_REGISTRY裡的
   版位(不是下面ensureDynamicLayoutRegistered()那種同一頁重複實例的動態
   別名)，但它跟05_ddcard共用同一份背景圖/K數上限規則——這裡刻意用一個
   「跟window.LAYOUT_ALIAS_BASE不同」的獨立map記錄這件事：
   window.LAYOUT_ALIAS_BASE專門給「動態複製實例」用(例如'03_c2c_bn__2')，
   js/editor-popups.js的「追加版位」彈窗會把出現在LAYOUT_ALIAS_BASE裡的
   id從勾選清單排除(因為那些是同一版位重複出現的複製體，不是獨立種類)。
   如果05_ddcard_nologo也塞進LAYOUT_ALIAS_BASE，會被那個彈窗誤判成
   「05_ddcard的複製體」整個從勾選清單消失，使用者就沒辦法手動勾選這個
   版位——2026-08發現這個問題，改成獨立的window.LAYOUT_ASSET_FALLBACK
   這個map，只給modules/background-module.js、modules/mask-module.js、
   js/editor-export.js這幾個「查不到專屬背景圖/規則就retry回哪個id」的
   地方使用，不影響「追加版位」彈窗的勾選清單判斷。 */
...
var S = {
  ...
  shadowAngle: 'left',      // 光源角度('left'/'top'/'right')，2026-08修正：原本沒存進S/tab資料，
                            ...
                            // 角度重開popup會跳回預設值（雖然已經套用/攤平的圖片本身不受影響，
                            // 但如果重開popup後又調整了別的東西再重新套用，會不小心把角度也改回預設）。
                            // 2026-08再調整：預設角度從'top'(中)改成'left'(左)，使用者反映商品陰影
                            // 預設要往左。
...
function saveCurrentTabIntoData(){
  ...
  tab.data.templateVersion = normalizeTemplateVersion(S.templateVersion);
  ...
  tab.data.shadowAngle = S.shadowAngle || 'left';
  ...
  tab.data.stageEnabled = (typeof S.stageEnabled === 'boolean') ? S.stageEnabled : null;
...
function applyTabData(i, cb){
  ...
  S.templateVersion = normalizeTemplateVersion(d.templateVersion);
  ...
  S.shadowAngle = d.shadowAngle || 'left';
  ...
  S.stageEnabled = (typeof d.stageEnabled === 'boolean') ? d.stageEnabled : null;
