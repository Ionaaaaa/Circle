...
window.ShadowPlugin = (function () {
  ...
  // isTicket：電子票券繳費專案新增——券樣槽位(見coupon-ticket.js)不管type是不是
  // 'product'，一律改走drawEllipseShadow()（底部單純橢圓陰影），不套用一般商品
  // 那組貼地斜切陰影（券是扁平卡片，斜切/擠壓陰影套上去看起來會很奇怪，見使用者
  // 需求：「券樣的陰影想單獨做一個合適的」）。
  function registerProduct(id, imgEl, type, isTicket) {
    return new Promise(function (resolve) {
      function build() {
        var silhouette = buildSilhouette(imgEl);
        var trim = detectAlphaTrim(imgEl);
        products[id] = { img: imgEl, silhouette: silhouette, tinted: null, trim: trim, type: type || 'product', isTicket: !!isTicket };
        tintProduct(id);
        resolve(products[id]);
      }
  ...
  // ---- 券樣（電子票券繳費專案新增）：單純底部橢圓陰影，不做貼地斜切/擠壓 ----
  // 券是扁平卡片，跟商品那種有厚度、立起來的物件視覺邏輯不一樣，套用同一組
  // 斜切+擠壓陰影效果會不自然；改成一個柔和模糊的橢圓貼在券的正下方，不隨
  // 光源角度改變形狀（只有商品的貼地陰影會跟著光源角度斜切），比較像一張
  // 卡片安穩地平放在桌面上的感覺。陰影本身不隨券的旋轉角度轉動——跟商品的
  // drawGroundShadow同一個既有慣例（見上面「陰影不跟著商品旋轉」的說明），
  // 只有券的照片本體會旋轉。
  function drawEllipseShadow(ctx, id, state, occluderMask, skipPhoto) {
    var p = products[id];
    if (!p) return;
    ...
    var trimBottomPad = p.trim ? p.trim.bottom * ph : 0;
    var py = state.y + trimBottomPad;
    // 旋轉支點定位在素材左右／上下的正中間（不是底部，也不是一般商品
    // drawGroundShadow用的置中公式，這裡直接用trim校正後的py反推正中心，
    // 確保左右(cx)、上下都精準置中）。
    var pivotX = cx, pivotY = py - ph / 2;
    var rot = state.rot || 0;

    var ellW = pw * 0.86;
    var ellH = Math.max(8, ph * 0.14);
    var ellCx = cx;
    var ellCy = py - ellH * 0.25; // 稍微往上收，貼近券的下緣、不整條懸空

    var tmp = document.createElement('canvas');
    tmp.width = ctx.canvas.width; tmp.height = ctx.canvas.height;
    var tctx = tmp.getContext('2d');
    tctx.save();
    tctx.translate(ellCx, ellCy);
    tctx.scale(1, ellH / ellW);
    var grad = tctx.createRadialGradient(0, 0, 0, 0, 0, ellW / 2);
    grad.addColorStop(0, 'rgba(' + shadowRGB + ',0.65)');
    grad.addColorStop(0.7, 'rgba(' + shadowRGB + ',0.4)');
    grad.addColorStop(1, 'rgba(' + shadowRGB + ',0)');
    tctx.fillStyle = grad;
    tctx.beginPath();
    tctx.arc(0, 0, ellW / 2, 0, Math.PI * 2);
    tctx.fill();
    tctx.restore();

    if (occluderMask) {
      tctx.globalCompositeOperation = 'destination-out';
      tctx.globalAlpha = FIXED.occlude / 100;
      tctx.drawImage(occluderMask, 0, 0);
      tctx.globalCompositeOperation = 'source-over';
      tctx.globalAlpha = 1;
    }
  ...
  /* 券樣橢圓陰影開關——使用者反映先隱藏起來看看效果，改成false。程式碼/
     drawEllipseShadow()函式本身都還在，之後要恢復只要改回true即可，不用
     重寫。貼地陰影(drawGroundShadow)不受影響，維持照常顯示。 */
  var SHOW_TICKET_ELLIPSE_SHADOW = false;

  function drawItem(ctx, id, state, occluderMask, skipPhoto) {
    var type = getType(id);
    var p = products[id];
    if (type === 'person') {
      drawPersonGlow(ctx, id, state, occluderMask, skipPhoto);
    } else if (p && p.isTicket) {
      /* 使用者反映：橢圓陰影是額外加上去的效果，不是要取代掉原本商品共用
         的貼地陰影（跟其他曝品一樣會隨「陰影寬度/陰影長度」滑桿、光源角度
         調整）——所以券樣兩個陰影都要畫：先畫橢圓陰影(只畫陰影，
         skipPhoto=true，照片交給下面drawGroundShadow畫，不會畫兩次)，
         再照常畫一般商品的貼地陰影+照片本體，讓券樣可以跟其他曝品一起
         用同一組共用陰影參數編輯，不會因為isTicket就變成外於這套機制的
         特例。★2026-08：橢圓陰影先隱藏(SHOW_TICKET_ELLIPSE_SHADOW=false)。 */
      if(SHOW_TICKET_ELLIPSE_SHADOW) drawEllipseShadow(ctx, id, state, occluderMask, true);
      drawGroundShadow(ctx, id, state, occluderMask, skipPhoto);
    } else {
      drawGroundShadow(ctx, id, state, occluderMask, skipPhoto);
    }
  }
