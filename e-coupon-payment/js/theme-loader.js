...
function setTemplateVersion(v){
  var key = (typeof normalizeTemplateVersion === 'function') ? normalizeTemplateVersion(v) : ((v === 'B') ? 'B' : 'A');
  ...
  /* 電子票券繳費專案新增：這個版本有沒有舞台(hasStage)，切版本時同步重新
     載入/清空舞台圖，見js/shadow-system/shadow-layout-receiver.js的
     reloadStageForVersion()說明。 */
  if(window.ShadowLayoutReceiver && window.ShadowLayoutReceiver.reloadStageForVersion){
    window.ShadowLayoutReceiver.reloadStageForVersion(key);
  }
}
