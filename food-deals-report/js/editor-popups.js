...
function runImport(excelFile, folderFiles){
  ...
  function afterParsed(parsed){
    ...
    var matched = (folderFiles && folderFiles.length)
      ...
      : {};
    addTabFromImport(parsed || { orderName:'未命名工單', text:{}, materials:[] });
    tabCreated = true;
    applyArFromImport(matched, parsed && parsed.text, parsed && parsed.arInfo);
    ...
    function goToShadowStep(){
      renderAll();
      proceedToShadowFromImport(matched, exposure);
    }
  ...
  if(excelFile){
    importCircleExcel(excelFile).then(afterParsed).catch(function(err){
      if(tabCreated){
        /* 分頁已經建立成功了，是後面的流程（LOGO/商品確認popup那段）出錯，
           不是Excel解析本身失敗——只提示錯誤，不要再呼叫一次afterParsed(null)
           重新建立一個內容一樣的分頁。 */
...
/* 資料夾裡比對到的人物/商品檔案，不再用「靜態預覽+廣播位置」這套（那套只適合
   ...
   組合怎麼決定，優先順序：
     ...
     2. 猜不到才退回 guessComboFromMatchedSlots()，依照資料夾實際比對到哪些
        槽位反推一個最合理的組合 */
function proceedToShadowFromImport(matched, exposure){
  ...
  if(!matchedShadowKeys.length){
    ...
    openShadowPopup(); // 沒比對到任何檔案，還是要開popup，只是裡面是空的
    return;
  }
