...
var EXPORT_SIZE_LIMIT_KB = {
  ...
  /* 2026-08確認：蝦皮家居-寢具的MSBN沒有檔案大小限制，先前照工單K數欄位
     ...
     EXPORT_JPEG_NO_LIMIT——固定用高畫質JPEG、不用試壓縮找KB上限。 */
};
...
/* MSBN沒有KB大小上限，但下載檔名規則是msbnN.jpg（副檔名固定.jpg），所以
   歸在這組「JPEG、不用試壓縮找臨界值，固定用高畫質輸出」——跟LPBN_APP/PC/IG
   同一套邏輯：只規定格式、不規定檔案大小。 */
var EXPORT_JPEG_NO_LIMIT = { '11_lpbn_app': true, '12_lpbn_pc': true, '04_ig': true, '07_msbn': true };
...
function downloadSingle(layoutId){
  ...
  /* MSBN家族（msbn1、msbn2...）下載檔名有明確規則：只用小寫的
     exportName本身，不加序號前綴——跟其他版位「編號_版位名稱.ext」的
     慣例不同，這裡特別分開處理，其他版位完全不受影響。 */
  var isMsbn = isMsbnFamilyId(layoutId);
  var namePart = layout ? (isMsbn ? (layout.exportName || layout.name) : layout.name) : layoutId;
  var prefix = (idx && !isMsbn) ? (idx+'_') : '';
...
function buildTempSavePayload(){
  /* 2026-08修正「暫存檔缺曝光資源，只剩MSBN」：這裡以前會呼叫一次
     saveCurrentTabIntoData()，想法是「保險起見再存一次最新的S」。但這支
     函式被呼叫的時機是downloadAll()整個匯出迴圈跑完之後——迴圈跑完時，
     全域的S其實還停留在「最後一個被exportOneTab()處理過的分頁」（通常
     就是msbn分頁，因為它在TABS陣列排在最後），而ACTIVE_TAB這個變數整個
     迴圈期間完全沒被改過，還是使用者按下「整包下載」當下所在的那一頁。
     這裡再呼叫saveCurrentTabIntoData()，等於把「msbn分頁的S」硬寫回
     「使用者原本所在那一頁」的tab.data，把原本正確的曝光資源資料整個
     覆蓋成msbn的資料——這就是暫存檔重新匯入後只剩MSBN、曝光資源整頁消失
     的原因。
     真正需要的「存最新的S」，downloadAll()一開始（迴圈開始之前）就已經
     呼叫過一次saveCurrentTabIntoData()了，那次呼叫時S/ACTIVE_TAB完全對得
     起來，是正確的存檔時機。這裡不用再存一次，直接讀目前TABS陣列（迴圈
     過程中只有「讀」applyTabData()，不會寫回TABS，所以每個分頁在TABS裡
     的資料從頭到尾都是正確的）就好。 */
  return { version:1, tabs: TABS.map(function(t){ return t.data; }), activeTab: ACTIVE_TAB, bgVersion: PROJECT_BG_VERSION };
}
  ...
  confirmDownloadAllWithComplianceCheck(function(){
      ...
      applyTabData(i, function(){
        buildCanvasArea().then(function(){
          applyDefaultLogos(function(){
            ...
            /* 使用者要求：第一個分頁(一般版位，DD Card/HBN/LPBN這些)不用
               另外包資料夾，檔案直接放在zip最外層；只有MSBN那個固定分頁
               才需要包成子資料夾（跟其他版位的檔案分開，方便辨識）。用
               tab.data._isMsbnTab判斷，不是用「第幾個分頁」這種位置寫死
               的判斷方式，之後就算分頁順序有變也不會判斷錯。 */
            var isMsbnTab = !!tab.data._isMsbnTab;
            var target = zip;
            if(isMsbnTab){
            ...
            var jobs = order.map(function(layout, idx){
              ...
              return exportLayoutBlob(layout.id, canvas).then(function(res){
                /* 整包下載時，MSBN家族一樣套用msbnN.jpg命名規則，不要編號前綴。 */
                var filename = isMsbnFamilyId(layout.id)
                  ? sanitizeFilename(layout.exportName || layout.name)+'.'+res.ext
                  : (idx+1)+'_'+sanitizeFilename(layout.name)+'.'+res.ext;
                target.file(filename, res.blob);
              });
            });
    ...
    exportOneTab(0, function(){
      ...
      zip.generateAsync({type:'blob'}).then(function(blob){
        /* zip檔本身的檔名：改成固定用「主要工單分頁」的名稱，不要跟著
           使用者按下「整包下載」當下所在哪一頁而變動——原本用
           TABS[originalActiveTab]，如果剛好是在「msbn」那個固定分頁上
           按下載，zip檔案本身就會被命名成「MSBN.zip」，即使裡面其實兩個
           分頁的內容都有包，使用者光看外層檔名也會誤以為「這包只有MSBN」。
           改成優先找「不是msbn固定分頁」的第一個分頁(通常就是主要的
           工單分頁)當檔名依據，固定不受目前在哪一頁影響；真的找不到
           （理論上不會發生，msbn分頁一定有個伴）才退回用原本在看的那頁。 */
        var mainTab = TABS.filter(function(t){ return !t.data._isMsbnTab; })[0] || TABS[originalActiveTab];
        ...
        var tabName = mainTab ? (mainTab.data.baseName || mainTab.data.label) : '未命名工單';
