...
function buildCanvasArea(){
  ...
  var loaders = activeLayouts().map(function(layout){
    ...
    block.innerHTML =
      '<div class="canvas-meta" data-layout-id="'+layout.id+'" style="cursor:pointer;" title="點這裡：右側文案面板切換成這個版位對應的那組文案">'+
        ...
        (isMsbnFamilyId(layout.id) && msbnVersionNumberFromId(layout.id) !== 1 ?
          '<button class="mini-dl-btn" style="color:#b94a3d;" onclick="event.stopPropagation();removeMsbnVersion(\''+layout.id+'\')">✕ 刪除此版本</button>' : '')+
      '</div>'+
      '<div class="canvas-wrap'+(isMsbnTabOnlyId(layout.id) ? ' msbn-canvas-wrap' : '')+'"><canvas id="cv-'+layout.id+'"></canvas></div>';
    ...
    attachMsbnTextInteraction(canvases[layout.id], layout.id);
  ...
  return Promise.all(loaders).then(function(){
    /* 2026-08新增：這個版位除了'host'以外，如果還有其他有artZone的獨立
       商品區塊(例如popup的'popupHost')，也要各自掛一份拖曳/縮放互動——
       一定要等上面Promise.all(loaders)真的把bundle載入完成之後才能做，
       _hostAssetKeysFor()要讀bundle.positions.assets才查得出有哪些key，
       在那之前bundles[layout.id]還是undefined，查出來一定是空陣列。 */
    activeLayouts().forEach(function(layout){
      _hostAssetKeysFor(layout.id).filter(function(k){ return k !== 'host'; }).forEach(function(extraKey){
        attachHostDragResize(canvases[layout.id], layout.id, extraKey);
      });
    });
    renderAll();
    buildAssetList();
    ...
    highlightCanvasBlocksForActiveGroup();
    if(typeof updateMsbnIssueBadge === 'function') updateMsbnIssueBadge(); // 每次重建畫布(切分頁/匯入/新增版本)都重新檢查一次MSBN文字問題，見js/msbn-text-compliance.js
  });
}
...
/* ══════════════════ 直接在主畫布上拖曳/縮放商品(host) ══════════════════
   ...
   2026-08擴充：原本這整套(getHostBox/drawHostOverlay/commitHostPos/
   attachHostDragResize)寫死只認'host'這一個素材key。popup版位新增了
   獨立的'popupHost'之後，這裡全部加上assetKey參數(預設'host'，不傳的話
   跟以前行為完全一樣)，呼叫端(renderAll()/buildCanvasArea())改成掃描
   這個版位「所有」有artZone的素材key、每個都各自掛一份拖曳/縮放互動，
   不再只認host一種——這樣popupHost也能像host一樣直接在畫布上拖曳/縮放，
   不用被迫走「調整位置」popup。_hostSelected/_hostInteraction的key改用
   layoutId+':'+assetKey組合，避免同一個版位如果未來有兩個以上獨立商品區
   互相打架(目前還沒有這種情況，但先做對，以後不用再改一次)。 */
var _hostSelected = {};    // (layoutId+':'+assetKey) -> boolean，目前是否顯示選取框/控制點
var _hostInteraction = null; // 目前拖曳中的互動狀態（同時只會拖一個）

/* 這個版位裡，所有「有artZone(需要拖曳/縮放互動)」的素材key——跟
   ensureHostAutoFit()掃描的條件完全一致，兩邊要用同一份判斷式，不然
   會出現「這裡當作品有拖曳互動，但自動貼合沒處理」或反過來的不一致。 */
function _hostAssetKeysFor(layoutId){
  var bundle = bundles[layoutId];
  if(!bundle || !bundle.positions || !bundle.positions.assets) return [];
  return Object.keys(bundle.positions.assets).filter(function(k){
    return bundle.positions.assets[k] && bundle.positions.assets[k].artZone;
  });
}
...
function getHostBox(layoutId, assetKey){
  ...
  var pos = merged.assets && merged.assets[assetKey];
  if(!pos) return null;
...
function drawHostOverlay(canvas, layoutId, assetKey){
  assetKey = assetKey || 'host';
  var selKey = layoutId + ':' + assetKey;
  if(!_hostSelected[selKey]) return;
  var box = getHostBox(layoutId, assetKey);
  if(!box) return;
  var cb = box.colorBox;
...
function renderLayoutClean(layoutId){
  ...
  if(bundle && canvas){
    ...
    /* 2026-09新增isExport:true——給modules/msbn-logo-module.js的
       msbnLogoSlot.draw()判斷用，讓它跳過「點這裡可以開陰影編輯器」那個
       圓形徽章＋鉛筆圖示的提示icon。那個icon純粹是編輯器介面的操作提示，
       不是版面設計的一部分，但因為是直接畫在跟畫面上一模一樣的canvas
       裡(msbnLogoSlot.draw()本身的邏輯，不是renderAll()額外疊加的覆蓋層)，
       renderLayoutClean()原本「重畫一次乾淨版本」只清掉了選取框/控制點
       這種renderAll()疊加層，沒有涵蓋到「模組自己畫的提示icon」這一種，
       才會被使用者發現匯出的圖檔上還留著那顆鉛筆圖示。 */
    var renderState = Object.assign({}, S, { text: groupText, isExport: true });
    Core.render(canvas, bundle, renderState, layoutId);
  }
}

function commitHostPos(layoutId, pos, assetKey){
  assetKey = assetKey || 'host';
  ...
  /* 直接在畫布上拖曳/縮放商品(不透過「調整位置」popup)，一樣要同步給
     ...
     2026-08修正：popupHost是每個分頁各自獨立的商品，不像host有「共用
     商品」的概念，這個跨分頁同步只給'host'用，其他assetKey跳過。 */
  if(assetKey === 'host') propagateLayoutPositionToLinkedTabs(layoutId, S.positionOverrides[layoutId]);
  renderAll();
}

function attachHostDragResize(canvas, layoutId, assetKey){
  assetKey = assetKey || 'host';
  var selKey = layoutId + ':' + assetKey;
  ...
  canvas.addEventListener('pointerdown', function(e){
    var box = getHostBox(layoutId, assetKey);
    if(!box){
      if(_hostSelected[selKey]){ _hostSelected[selKey] = false; renderAll(); }
      return;
    }
    var p = toCanvasPos(e);
    var cb = box.colorBox;
  ...
  canvas.addEventListener('pointermove', function(e){
    ...
    if(!it || it.layoutId !== layoutId || it.assetKey !== assetKey) return;
    e.preventDefault();
    var p = toCanvasPos(e);
    ...
    commitHostPos(layoutId, newPos, assetKey);
  });
  ['pointerup','pointercancel'].forEach(function(evt){
    canvas.addEventListener(evt, function(){
      if(_hostInteraction && _hostInteraction.layoutId === layoutId && _hostInteraction.assetKey === assetKey) _hostInteraction = null;
    });
  });
}
...
/* 2026-08擴充：原本這支函式寫死只認'host'這一個素材key，每個版位最多
   ...
   同一套貼合/防溢出/防超出畫布邏輯，S.assets[key]對應到positions.assets
   [key]，S.positionOverrides[layoutId].assets[key]各自存各自的——這樣
   'host'(LPBN/HBN/DD Card共用)、'popupHost'(popup專用，見configs/layouts/
   08_popup-positions.json)兩組完全獨立運作，互不影響。原本只有'host'一組
   的版位行為完全不變(迴圈跑一次，結果跟以前一樣)。 */
function ensureHostAutoFit(){
  activeLayouts().forEach(function(layout){
    var bundle = bundles[layout.id];
    if(!bundle || !bundle.positions || !bundle.positions.assets) return;

    Object.keys(bundle.positions.assets).forEach(function(assetKey){
      var baseHost = bundle.positions.assets[assetKey];
      ...
      ov.assets = ov.assets || {};
      if(ov.assets[assetKey]) return; // 這個欄位已經貼合過(或使用者調整過)一次了，不管圖片有沒有換都不再自動蓋掉
...
function buildLayoutToggleList(){
  ...
  body.innerHTML = LAYOUT_REGISTRY.filter(function(l){ return !aliasBase[l.id] && !l.hiddenFromToggle; }).map(function(l){
...
function loadDefaultArActivityLogo(cb){
  ...
  img.onerror = function(){ if(cb) cb(); };
  img.src = 'logos/ar-activity-logo.png';
}
...
/* CTA固定素材：兩種badge分別對應不同版位，不能共用同一個assets key——
   ...
   logos/CTA.png（逛逛去，3字大顆款）固定套用在MSBN公版一、五、六這種CTA
   位置尺寸較大(約200x71)的版型；logos/MsbnIconCTA.png（單顆小方形icon款，
   還沒放圖）固定套用在MSBN公版二、三、四這種CTA位置尺寸較小(約63~71見方)
   的版型。跟logo1同一套邏輯：只是「有預設檔就套用、找不到對應layer就
...
/* 2026-08新增：MSBN公版二~四這種「單顆小方形icon」CTA，使用者明確表示
   ...
   ctaGo/ctaGame同一套「有預設檔就套用、找不到就靜靜跳過」邏輯，固定檔名
   logos/MsbnIconCTA.png——這張圖目前還沒有放進logos/資料夾，在補上之前
   這幾個版型的CTA位置會維持空白（不會報錯，只是暫時看不到CTA），等圖片
   一放進去就會自動生效，不用改任何程式碼。
   ctaMsbnIconA/B/C三個別名：因為modules/logo-module.js是「一個assets key
   對應一個固定位置」，公版三(2顆)、公版四(3顆)同一張圖要畫在好幾個不同
   位置，不能共用同一個assets key（共用的話後面的position會蓋掉前面
   的，變成兩顆疊在同一個地方）。這裡load一次，同時指派給
   ctaMsbnIcon(向下相容/公版二單顆用)、ctaMsbnIconA/B/C(公版三、四每一顆
...
/* 固定預設素材一起套用（沒有對應檔案就靜靜跳過），每次分頁切換/建立/載入完
   ...
   沒有logo1/logo2/ctaDD/ctaGo/ctaGame/ctaMsbnIcon的話，才會補上預設值 */
function applyDefaultLogos(cb){
  loadDefaultLogo1(function(){ loadDefaultArActivityLogo(function(){ loadDefaultLogo2(function(){ loadDefaultCTA_DD(function(){ loadDefaultCTA_Go(function(){ loadDefaultCTA_Game(function(){ loadDefaultCTA_MsbnIcon(cb); }); }); }); }); }); });
}
...
/* 2026-08新增：msbn分頁的標題/副標/日期完全不會用到S.textGroups這組通用
   輸入框——MSBN的文字是直接點畫布打字、存在S.msbnTexts裡（見
   js/msbn-text-interaction.js），跟這幾個輸入框完全無關，留著只會讓
   使用者誤以為填了這裡畫布就會有反應，容易搞混。改成：只要目前是msbn
   分頁，就把這組輸入框(#normal-text-fields)整個隱藏，只留上面的禁用語
   徽章／點開徽章看到的問題清單(#msbn-issues-panel，那個本來就是跟
   msbnTexts走的，不受影響)。一般分頁(有用到S.textGroups的)則維持原樣
   顯示。 */
function updateNormalTextFieldsVisibility(){
  var isMsbnTab = !!(TABS[ACTIVE_TAB] && TABS[ACTIVE_TAB].data && TABS[ACTIVE_TAB].data._isMsbnTab);
  ['normal-text-fields','general-asset-fields','general-hint-fields'].forEach(function(id){
    var el = document.getElementById(id);
    if(el) el.style.display = isMsbnTab ? 'none' : '';
  });
}
...
function isEmptyDefaultTabData(d){
  ...
  /* logo1／cta都有固定預設值（logo1是蝦皮直播brand、cta是DD.png固定badge），
     ...
     2026-08修正：新增ctaMsbnIcon這個固定預設素材(見js/editor-main.js的
     loadDefaultCTA_MsbnIcon())之後，忘記把它也加進這個allowlist——
     logos/MsbnIconCTA.png補上之後，每個分頁(包括根本還沒被使用者碰過的
     占位分頁)一開機就會自動帶入這個值，沒加進allowlist的話assetsEmpty
     又會被判定成false，「匯入變成多一個空白分頁」這個bug用同樣的原因
     再次重現(這次是ctaMsbnIcon，不是logo1/cta)——這正是使用者回報「又
     出現一個叫做未命名工單1的分頁」的根本原因。
     2026-08再次修正：同樣的問題還藏著第二個——arActivityLogo(AR版位的
     活動LOGO，見loadDefaultArActivityLogo())也是「有檔案就自動套用」的
     固定預設素材，一樣漏了加進這個allowlist，logos/ar_activity_logo.png
     一旦真的補上了圖檔，會用一模一樣的原因讓「多一個空白分頁」這個bug
     重現第三次。這類bug的根本規律是：以後任何一個新增的「固定預設會自動
     套用」素材，都要記得同步加進這裡——這個allowlist本質上是在回答「這個
     key即使有值，也不代表使用者真的動過這個分頁」，凡是「開機就自動有
     值、不需要使用者操作」的素材都屬於這一類。 */
  /* 2026-08再修正：loadDefaultCTA_MsbnIcon()其實一次會塞四個key
     (ctaMsbnIcon本尊+ctaMsbnIconA/B/C三個別名，見該函式說明)，這裡先前
     只排除了ctaMsbnIcon本尊，漏掉A/B/C——只要logos/MsbnIconCTA.png這個
     檔案存在，任何空白占位分頁一開機就會被自動塞進這三個key，
     assetsEmpty被誤判成false，「匯入變成多一個空白分頁」這個bug用
     同樣的根本原因（allowlist漏掉某個固定自動套用的key）第四次重現。
     這裡直接把三個別名一起補上，跟本尊套同一條規則。 */
  var assetsEmpty = !d.assets || Object.keys(d.assets).every(function(k){ return k === 'logo1' || k === 'ctaDD' || k === 'ctaGo' || k === 'ctaGame' || k === 'ctaMsbnIcon' || k === 'ctaMsbnIconA' || k === 'ctaMsbnIconB' || k === 'ctaMsbnIconC' || k === 'arActivityLogo' || !d.assets[k]; });
...
function buildTabDataFromParsedBlock(parsed){
  ...
  /* 使用者要求：MSBN固定只出現在專屬的「msbn」分頁，Excel匯入出來的一般
     ...
     固定分頁了)。 */
  data.activeLayoutIds = data.activeLayoutIds.filter(function(id){ return MSBN_BASE_IDS.indexOf(id) === -1; });
  data.instances = built.instances.filter(function(inst){ return !isMsbnFamilyId(inst.layoutId); });
...
function bindSaveTemp(){
  document.getElementById('btn-save-temp').onclick = function(){
    saveCurrentTabIntoData();
    /* 2026-08(KRCB)：bgVersion是專案層級的全域設定(見js/editor-state.js
       的PROJECT_BG_VERSION說明)，不屬於任何一個分頁，存暫存檔時要跟
       tabs/activeTab平行存一份，不是塞進某個分頁的data裡。 */
    var payload = { version:1, tabs: TABS.map(function(t){ return t.data; }), activeTab: ACTIVE_TAB, bgVersion: PROJECT_BG_VERSION };
...
/* ══════════════════ MSBN：從工單資料自動比對＋載入LOGO素材圖 ══════════════════
   ...
   兩件事要做：
     ...
     2) 如果使用者這次匯入有附資料夾(folderFiles)，各自用三個名稱去資料夾
        裡找檔案，找到的話各自載入對應的格子。 */
/* 2026-08(KRCB)：knownRealLayoutId(選填)——Excel匯入時呼叫端已經明確
   知道這個實例真正要套用的版型是什麼(例如'07_msbn_v2')，直接傳進來，
   在getMsbnSlotBox()因為bundle還沒載入而查不到框時，可以用這個去查
   MSBN_SLOT_BOX_FALLBACK靜態備援表算出正確的contain-fit縮放，不會用
   圖片原始尺寸當框(那會導致商品匯入進來過大，超出作圖範圍)。使用者手動
   點擊上傳(js/msbn-logo-interaction.js)這種情境，畫布已經渲染過，
   getMsbnSlotBox()一定查得到，不需要傳這個參數。 */
function loadMsbnLogoFileInto(targetLogosObj, instanceId, slotKey, file, cb, knownRealLayoutId){
  var reader = new FileReader();
  reader.onload = function(ev){
    var img = new Image();
    img.onload = function(){
      var slotBox = getMsbnSlotBox(instanceId, slotKey);
      var fallbackBox = (!slotBox && knownRealLayoutId && typeof getMsbnSlotBoxFallback === 'function') ? getMsbnSlotBoxFallback(knownRealLayoutId, slotKey) : null;
      var box = slotBox ? slotBox.logoBox : fallbackBox;
      /* 2026-09修正「商品/LOGO嚴重超出範圍」：跟js/shadow-system/
         shadow-popup.js的autoComposeMsbnHost()同一個地雷——box查不到
         (slotBox跟fallbackBox都是null)的時候，原本會退回用
         { w: img.naturalWidth, h: img.naturalHeight }（圖片自己的原始
         尺寸）當作框。這裡雖然公式看起來人畜無害(baseScale算出來剛好
         是min(1,1)=1，不像host那邊會衝到2倍以上)，但「scale=1」代表
         直接用「使用者上傳照片的原始像素尺寸」去塞進格子——手機拍的
         照片動輒3000~4000px寬，格子(logoBox)通常只有一兩百到幾百px，
         套用scale=1等於把一張超大照片硬塞進一個小框，一樣是嚴重超出、
         只看得到局部特寫。
         這個地雷目前最容易踩到的地方：公版三~八(07_msbn_v3~v8)的logo1/
         logo2/logoLeft/logoRight這幾個一般LOGO欄位——MSBN_SLOT_BOX_
         FALLBACK備援表目前只有host/host1/host2/icon1~3這4組資料，這幾
         版完全沒有備援，只要bundle還沒載入(例如Excel剛匯入完、使用者
         還沒切去看過那個MSBN分頁)，一定會查不到框、踩到這個地雷。
         修法：查不到框就不要假裝「圖片原始尺寸」是框，改用保守預設值
         (0.3，比大多數格子安全)頂著，並印出明確警告方便之後排查是哪個
         instanceId/slotKey查不到框——最多這一格看起來偏小，不會出現
         完全無法辨識的超大特寫。 */
      var baseScale;
      if(box){
        ...
        console.warn('[loadMsbnLogoFileInto] 查不到格子(logoBox)大小，無法計算正確縮放比例，改用保守預設值——instanceId=', instanceId, 'slotKey=', slotKey);
        baseScale = 0.3;
      }
      /* 跟logo2同一套底色判斷：PNG固定白色；JPG等本身有背景的格式，
         ...
         「算」都不算，省掉沒必要的取樣運算。 */
      /* MSBN_SLOT_BOX_FALLBACK裡目前所有欄位(host/host1/host2/icon1~3)
         都是「商品/ICON去背透明底」性質，一律不鋪底色——用了備援框
         (fallbackBox存在)就直接視為noBgFill:true，不用另外維護一份
         「這個備援欄位是不是LOGO卡片」的清單。 */
      var bgColor = (slotBox ? slotBox.noBgFill : !!fallbackBox) ? null : logo2SampleBgColor(img);
...
/* 2026-08重寫：原本這支函式假設每個MSBN版本永遠是固定的left/mid/right
   三格LOGO，現在slotNames(來自parseMsbnVersionLegacy()的新格式)改成
   { layoutId, logos:{slotKey:name}, texts:{slotKey:內容} }——每個版本用
   哪個版型、有幾格、格子叫什麼名字都不再寫死，直接照Excel「版型」欄位
   實際指定的走。
   這支函式現在要做三件事：
     1. 修正這個實例真正該用的layoutId(如果跟目前預設的'07_msbn'不一樣)——
        改完之後下次activeLayouts()/buildCanvasArea()跑到這個實例時，
        ensureDynamicLayoutRegistered()會偵測到layoutId變了，自動更新
        LAYOUT_REGISTRY裡對應的alias項目(見js/editor-state.js的說明)，
        背景圖/排版/LOGO格數就會正確換成這個版型該有的樣子
     2. 把LOGO/圖片類的欄位放進S.msbnLogos（原本就有的機制，只是slotKey
        不再限定left/mid/right）
     3. 把文案類的欄位放進S.msbnTexts（2026-08新增的獨立文字系統，見
        js/msbn-text-interaction.js） */
/* 2026-08(KRCB)新增：MSBN商品欄位一次匯入多張圖片(主商品+贈品等)時，
   預先把資料填進這一格的持久化合成狀態(S.msbnShadowSlots，見
   js/shadow-system/shadow-popup.js的openMsbnHostComposePopup()說明)，
   下次使用者點開「編輯商品」就會看到這些素材已經排好。
   固定用combo'D'(純商品組合，見js/shadow-system/shadow-layout-defaults-
   circle.js的CIRCLE_COMBO_SLOTS.D＝['商品2(中)','商品1(左)','商品3(右)'])，
   最多3品，依Excel欄位裡填的順序依序對應——超過3個名稱的部分會被忽略
   (目前combo系統最多只支援3品)。 */
var MSBN_MULTI_PRODUCT_SLOT_IDS = ['商品2(中)', '商品1(左)', '商品3(右)'];

function _applyMsbnMultiProductPrefill(instanceId, slotKey, dataUrls, isActive, tabIndex){
  var shadowSlots = {};
  dataUrls.slice(0, MSBN_MULTI_PRODUCT_SLOT_IDS.length).forEach(function(dataUrl, i){
    shadowSlots[MSBN_MULTI_PRODUCT_SLOT_IDS[i]] = { dataUrl: dataUrl, type: 'product', ratio: undefined, transform: undefined };
  });
  var key = (typeof _msbnComposeStateKey === 'function') ? _msbnComposeStateKey(instanceId, slotKey) : (instanceId + '::' + slotKey);
  var state = {
    shadowCombo: 'D',
    ...
    stageTransform: null,
    stageEnabled: true,
    kvElementEnabled: false
  };
  if(isActive){
    S.msbnShadowSlots = S.msbnShadowSlots || {};
    S.msbnShadowSlots[key] = state;
  } else {
    var tabData = TABS[tabIndex].data;
    tabData.msbnShadowSlots = tabData.msbnShadowSlots || {};
    tabData.msbnShadowSlots[key] = state;
  }

  /* 2026-08(KRCB)新增：使用者反映「畫面上應該先直接顯示商品，真的需要
     加陰影再點進去合成工具」——原本這裡只有預先準備好合成工具要用的
     素材(上面那段)，商品格子本身在使用者實際點開合成工具、按下確認之前
     完全是空的，畫面上什麼都看不到。改成：只要有比對到至少一張圖，
     立刻組一張「並排」的無陰影預覽直接顯示在格子裡——這樣使用者匯入完
     馬上就能在畫面上看到東西(而且多品是並排、不是疊在一起)，不用先點開
     合成工具才看得到；如果要幫這個商品加陰影/調整疊放，再點進去用合成
     工具，確認後才會換成有陰影疊放效果的最終版本。
     2026-08再修正：原本只拿「第一張」直接套用，其餘幾張完全被忽略，
     使用者反映「多品應該要並排放」——改成不管有幾張(1~3張)，都先各自
     載入，再畫一張「左右並排、各自contain-fit佔80%」的合成圖，當成
     這個格子的無陰影預覽(單一張圖，跟一般商品照片上傳套用的資料格式
     完全一樣，之後可以正常拖曳/縮放/選取整體)。 */
  if(dataUrls.length){
    var slotBoxForPreview = (typeof getMsbnSlotBox === 'function') ? getMsbnSlotBox(instanceId, slotKey) : null;
    var fallbackBoxForPreview = (!slotBoxForPreview && typeof getMsbnSlotBoxFallback === 'function')
      ? getMsbnSlotBoxFallback((window.LAYOUT_ALIAS_BASE && window.LAYOUT_ALIAS_BASE[instanceId]) || instanceId, slotKey)
      : null;
    var previewBox = (slotBoxForPreview && slotBoxForPreview.logoBox) || fallbackBoxForPreview || { w: 400, h: 400 };

    _buildSideBySideFlatPreview(dataUrls, previewBox, function(compositeDataUrl){
      if(!compositeDataUrl) return; // 全部圖片都載入失敗，維持空白，等使用者自己上傳
      var img = new Image();
      img.onload = function(){
        var baseScale = Math.min(previewBox.w/img.naturalWidth, previewBox.h/img.naturalHeight);
        var entry = { img: img, scale: baseScale, offX: 0, offY: 0, baseScale: baseScale, bgColor: null };
        if(isActive){
          S.msbnLogos = S.msbnLogos || {};
          S.msbnLogos[instanceId] = S.msbnLogos[instanceId] || {};
          S.msbnLogos[instanceId][slotKey] = entry;
          if(typeof renderAll === 'function') renderAll();
        } else {
          var tabData2 = TABS[tabIndex].data;
          tabData2.msbnLogos = tabData2.msbnLogos || {};
          tabData2.msbnLogos[instanceId] = tabData2.msbnLogos[instanceId] || {};
          tabData2.msbnLogos[instanceId][slotKey] = { src: img.src, scale: baseScale, offX: 0, offY: 0, baseScale: baseScale, bgColor: null };
        }
      };
...
/* 把最多3張圖片(dataURL字串)畫成「左右並排」的一張合成圖(canvas尺寸直接
   對齊box.w x box.h，畫完之後套進格子時baseScale會剛好貼齊、不用再另外
   縮放一次)——每張圖各自佔用box寬度平分後的其中一欄，欄內用contain-fit
   縮放到「這一欄的80%大小」置中(80%＝使用者要求的「至少占80%」，這裡欄
   本身已經是等分後的空間，80%是相對這個等分欄再縮小，不是相對整個box)，
   垂直方向也是contain-fit到box.h的80%、置中。
   全部圖片都要先個別載入完成(Promise.all風格的計數器)才畫，任何一張載入
   失敗就跳過那張(不會擋住其他張正常顯示)。cb(dataUrl|null)——null代表
   一張都沒載入成功。 */
function _buildSideBySideFlatPreview(dataUrls, box, cb){
  var loaded = new Array(dataUrls.length);
  ...
  function checkDone(){
    ...
    var ctx = canvas.getContext('2d');
    var okImgs = loaded.filter(Boolean);
    if(!okImgs.length){ cb(null); return; }
    var colW = canvas.width / okImgs.length;
    okImgs.forEach(function(img, i){
      var cellW = colW * 0.8, cellH = canvas.height * 0.8;
      var fitScale = Math.min(cellW/img.naturalWidth, cellH/img.naturalHeight);
      var drawW = img.naturalWidth * fitScale, drawH = img.naturalHeight * fitScale;
      var cx = colW*i + colW/2, cy = canvas.height/2;
  ...
  dataUrls.forEach(function(url, i){
    var img = new Image();
    img.onload = function(){ loaded[i] = img; pending--; checkDone(); };
    img.onerror = function(){ pending--; checkDone(); };
    img.src = url;
  });
  if(!dataUrls.length) cb(null);
}
...
function applyMsbnAssetsFromImport(slotNames, folderFiles){
  ...
  /* 把「LOGO/圖片類欄位→S.msbnLogos」「文案類欄位→S.msbnTexts」這兩段
     抽成獨立函式，直接吃instanceId(字串)——這樣「副區」這種要套用到
     固定實例(subzone_app/subzone_pc)、而不是套用到Excel那一列本身對應的
     編號實例的特殊情況，也能共用同一套邏輯，不用重複寫一次。 */
  function applyInfoToInstanceId(instanceId, info, realLayoutId){
    /* 2026-09新增：修正「Excel匯入自動套陰影後商品偏小，但點進去手動編輯
       再確認一次就正常」的根本原因——автoComposeMsbnHost()/loadMsbnLogoFileInto()
       查格子(logoBox)大小時，如果window.bundles裡還沒有這個實例的即時
       資料，會退回查getMsbnSlotBoxFallback(window.LAYOUT_ALIAS_BASE[instanceId]
       || instanceId, slotKey)這組靜態備援表。但window.LAYOUT_ALIAS_BASE
       [instanceId]這個對應關係，原本要等到ensureDynamicLayoutRegistered()
       被呼叫過才會存在——而這支函式只有在activeLayouts()(也就是實際畫布
       渲染流程buildCanvasArea())跑到這個實例時才會被呼叫。Excel剛匯入完、
       使用者還沒切去看過MSBN分頁的當下，這個對應關係根本還沒註冊，
       LAYOUT_ALIAS_BASE[instanceId]是undefined，備援表查詢直接用
       instanceId本身(例如'msbn_p1'這種動態id)去查，靜態表裡當然沒有這個
       key，box整個查不到，就會落到「查不到框」那個保守預設值(0.3)分支
       ——這才是「自動套用比手動確認小」的真正原因，不是size公式本身
       還有問題(手動確認之所以正常，是因為使用者點進那一頁時，
       buildCanvasArea()已經先幫忙註冊過這個對應關係了)。
       這裡我們已經知道正確的realLayoutId(是這個函式本來就有的參數)，
       不用等畫布渲染流程，直接現在就註冊，這樣不管是不是搶在畫布渲染
       之前呼叫autoComposeMsbnHost()，備援表都查得到正確的資料。 */
    if(typeof ensureDynamicLayoutRegistered === 'function' && instanceId !== realLayoutId){
      ensureDynamicLayoutRegistered(instanceId, realLayoutId);
    }
    Object.keys(info.logos || {}).forEach(function(slotKey){
      var names = info.logos[slotKey];
      if(!names || !names.length) return;

      /* 2026-08(KRCB)再修正：使用者要求「後台自動跑陰影合成，不用手動
         點開合成工具、按確認」——不管這格比對到1張還是2~3張圖，統一都
         走js/shadow-system/shadow-popup.js的autoComposeMsbnHost()，直接
         在背景跑完整套陰影合成引擎(跟手動開合成工具用的是同一份底層邏輯，
         只是不用使用者按任何按鈕)，商品格子匯入完就已經是「排好版、疊好
         陰影」的最終樣子，使用者事後只需要視需要放大/縮小；如果排版效果
         不滿意，一樣可以點格子上的鉛筆圖示，進手動合成工具接著調整(那邊
         看到的就是這次自動排好的內容)。
         這裡取代了之前「先只顯示第一張/或無陰影並排預覽」那兩版做法——
         舊版本的_applyMsbnMultiProductPrefill()/_buildSideBySideFlatPreview()
         函式定義還留著沒刪，只是這裡不再呼叫，之後如果想恢復「不自動加
         陰影」的行為，改回呼叫那兩支就好。 */
      if(slotKey.indexOf('host') === 0){
        var matchedUrls = []; // 依序收集比對到的每張圖片(dataURL字串)
        var pending = names.length;
        var finish = function(){
          var urls = matchedUrls.filter(Boolean);
          /* 2026-09新增：把realLayoutId一併傳進去，當作比LAYOUT_ALIAS_BASE
             查詢更直接、更不會受註冊時機影響的備援依據(雙保險，即使上面
             ensureDynamicLayoutRegistered因為某些原因沒生效，這裡還是有
             正確的realLayoutId可以用)。 */
          if(typeof autoComposeMsbnHost === 'function') autoComposeMsbnHost(instanceId, slotKey, urls, isActive, tabIndex, realLayoutId);
        };
        names.forEach(function(name, idx){
          var file = matchFileByAliases(imageFiles, [name]);
          if(!file){ pending--; return; }
          var reader = new FileReader();
          reader.onload = function(ev){
            matchedUrls[idx] = ev.target.result;
            pending--;
            if(pending <= 0) finish();
          };
          reader.readAsDataURL(file);
        });
        if(pending <= 0) finish(); // 一個檔案都比對不到，urls會是空陣列，autoComposeMsbnHost()看到空陣列會直接return，維持空白
        return;
      }
      ...
      var scratch = {};
      loadMsbnLogoFileInto(scratch, instanceId, slotKey, file, function(){
        var entry = scratch[instanceId] && scratch[instanceId][slotKey];
        if(!entry) return;
        if(isActive){
          S.msbnLogos = S.msbnLogos || {};
          S.msbnLogos[instanceId] = S.msbnLogos[instanceId] || {};
          ...
          tabData.msbnLogos = tabData.msbnLogos || {};
          tabData.msbnLogos[instanceId] = tabData.msbnLogos[instanceId] || {};
          tabData.msbnLogos[instanceId][slotKey] = {
    ...
    /* 2026-08(KRCB)新增：副區的ICON欄位——Excel填的是ICON「名稱」(例如
       「免運車」)，不是檔名，這個名稱要拿去比對data/icon-library.json
       這份後台圖庫清單(見js/icon-library.js的loadBundledIconManifest())，
       名稱對到才自動套用；比對不到就維持空白，等使用者自己從圖庫popup
       裡挑一個、或上傳一張新的(見js/msbn-logo-interaction.js的
       _msbnOpenPickerForSlot())。資料庫來的圖不套用固定顏色覆蓋(那是只
       給使用者自己上傳的圖用的，見js/icon-library.js的說明)，直接用
       原始檔案。 */
    Object.keys(info.icons || {}).forEach(function(slotKey){
      var iconName = info.icons[slotKey];
      if(!iconName) return;
      if(typeof loadBundledIconManifest !== 'function') return; // icon-library.js沒載入，直接跳過，不報錯

      loadBundledIconManifest().then(function(manifest){
        var match = manifest.filter(function(item){ return String(item.name||'').trim() === iconName; })[0];
        if(!match) return; // 資料庫沒有同名的ICON，維持空白

        var img = new Image();
        img.onload = function(){
          var slotBox = getMsbnSlotBox(instanceId, slotKey);
          var fallbackBox = (!slotBox && realLayoutId && typeof getMsbnSlotBoxFallback === 'function') ? getMsbnSlotBoxFallback(realLayoutId, slotKey) : null;
          var box = slotBox ? slotBox.logoBox : fallbackBox;
          /* 2026-09修正：跟loadMsbnLogoFileInto()同一個地雷——box查不到
             時不能退回用圖片自己的原始尺寸冒充框，改用保守預設值+警告。 */
          var baseScale;
          if(box){
            baseScale = Math.min(box.w/img.naturalWidth, box.h/img.naturalHeight);
          } else {
            console.warn('[applyMsbnAssetsFromImport/icons] 查不到格子(logoBox)大小，改用保守預設值——instanceId=', instanceId, 'slotKey=', slotKey);
            baseScale = 0.3;
          }
          var entry = { img: img, scale: baseScale, offX: 0, offY: 0, baseScale: baseScale, bgColor: null };
          if(isActive){
            ...
            var tabData3 = TABS[tabIndex].data;
            tabData3.msbnLogos = tabData3.msbnLogos || {};
            tabData3.msbnLogos[instanceId] = tabData3.msbnLogos[instanceId] || {};
            tabData3.msbnLogos[instanceId][slotKey] = { src: img.src, scale: baseScale, offX: 0, offY: 0, baseScale: baseScale, bgColor: null };
          }
        };
        img.src = ICON_LIBRARY_FOLDER + match.file;
      });
    });

    Object.keys(info.texts || {}).forEach(function(slotKey){
      var content = info.texts[slotKey];
      if(!content) return;
      if(isActive){
        S.msbnTexts = S.msbnTexts || {};
        S.msbnTexts[instanceId] = S.msbnTexts[instanceId] || {};
        S.msbnTexts[instanceId][slotKey] = content;
      } else {
        var tabData2 = TABS[tabIndex].data;
        tabData2.msbnTexts = tabData2.msbnTexts || {};
        tabData2.msbnTexts[instanceId] = tabData2.msbnTexts[instanceId] || {};
        tabData2.msbnTexts[instanceId][slotKey] = content;
      }
    });
  }

  /* 2026-08(KRCB)新增：「副區」在Excel裡是掛在某個「MSBNn」標籤底下
     (通常是MSBN1)寫的，但它實際上不是「這個編號的版型」，是固定存在的
     subzone_app/subzone_pc這兩個實例——不管Excel把它寫在MSBN1、MSBN2、
     或哪個編號底下，只要版型是副區，都是套用到這兩個固定實例，不會佔用
     「這個編號」。見js/editor-import.js的MSBN_TEMPLATE_SLOT_MAP['副區']，
     info.layoutId在這個情況下會是陣列(['subzone_app','subzone_pc'])，
     不是字串。
     被副區佔用掉的那個編號實例(例如msbn_p1)不需要留著——維持空白掛在
     那裡只會多一張沒人要用的空白MSBN卡片，這裡收集起來、等迴圈跑完後
     整批從instances陣列移除，不會佔用编號、也不會留下垃圾卡片。
     2026-08(KRCB)再修正「MSBN要照工單開幾個就寫幾個」：buildMsbnTabData()
     開機時固定生MSBN_DEFAULT_COUNT(6)份實例當殼，但實際工單通常只會用到
     其中幾個(這次工單「總製作內容」的MSBN只有2個，Excel也只寫了MSBN2、
     MSBN3兩筆)——沒被Excel任何一筆資料點名到的編號實例(例如MSBN4/5/6)，
     不應該維持空白掛在那裡，要一併移除，不是只有被副區佔用的那筆才移除。
     判斷依據：inst.label符合'MSBN\\d+'格式(排除subzone_app/subzone_pc，
     它們的label是null，不會誤判)、而且slotNames裡完全沒有對應的key。 */
  var instanceIdsToRemove = [];

  instances.forEach(function(inst){
    var info = slotNames[String(inst.label||'').toUpperCase()];
    if(!info){
      if(typeof inst.label === 'string' && /^MSBN\d+$/i.test(inst.label)){
        instanceIdsToRemove.push(inst.instanceId);
      }
      return;
    }

    if(Array.isArray(info.layoutId)){
      info.layoutId.forEach(function(targetId){ applyInfoToInstanceId(targetId, info, targetId); });
      instanceIdsToRemove.push(inst.instanceId);
      return;
    }

    inst.slotNames = info; // 存起來給畫布提示文字用，即使沒比對到檔案也留著

    /* ① 修正這個實例真正該用的版型——不寫的話畫面永遠停在
       buildMsbnTabData()當初給的預設值(07_msbn)，這是使用者回報「msbn
       背景/排版都對不上、都停在同一個版型」的根本原因。 */
    if(info.layoutId && info.layoutId !== inst.layoutId){
      inst.layoutId = info.layoutId;
      if(isActive){
        if(S.activeLayoutIds.indexOf(info.layoutId) < 0) S.activeLayoutIds.push(info.layoutId);
      } else {
        var tabActiveIds = TABS[tabIndex].data.activeLayoutIds;
        if(tabActiveIds && tabActiveIds.indexOf(info.layoutId) < 0) tabActiveIds.push(info.layoutId);
      }
    }

    applyInfoToInstanceId(inst.instanceId, info, info.layoutId);
  });

  if(instanceIdsToRemove.length){
    var arr = isActive ? S.instances : TABS[tabIndex].data.instances;
    for(var ri=arr.length-1; ri>=0; ri--){
      if(instanceIdsToRemove.indexOf(arr[ri].instanceId) !== -1) arr.splice(ri, 1);
    }
  }

  if(isActive) buildCanvasArea(); // 立刻重畫一次，讓版型/文案/「點擊上傳」提示文字換成正確內容（圖片載入完成後renderAll()會再畫一次真正的圖）
...
/* 2026-08訂正：這裡原本猜測「MSBN每一版固定對應哪個版型」是照順序位置
   自動決定的(第1版一定公版一、第2版一定公版二...)，实际比對真正的工單
   Excel資料後發現這個假設是錯的——版型是Excel自己的「版型」欄位直接
   指定的、每個位置可以自由選任何版型，不是跟位置序號綁死的固定對照表
   （例如實際工單MSBN1用的是公版二、MSBN6又繞回去用公版二，順序完全不
   是1→公版一、2→公版二這種遞增規律）。已拿掉這個錯誤的自動對照表——
   手動「＋新增MSBN版本」按鈕沒有Excel資料可以參考，新增出來的版本先
   固定用公版一(07_msbn)頂著，之後若有Excel匯入會依照「版型」欄位自動
...
function addMsbnVersion(baseLayoutId){
  ...
  /* 2026-08(KRCB)移除：這裡原本(海外購物節)是「新增的公版一版本，順手把
     main商品廣播過來」，因為當時公版一的host欄位沒有自己獨立的商品來源。
     KRCB的公版一已經改成從Excel「【Layout】」區塊自己的「圖片」欄位取得
     商品(見js/editor-import.js的MSBN_TEMPLATE_SLOT_MAP['版型1'])，不應該
     被main商品廣播蓋掉——手動用「＋新增MSBN版本」加出來的公版一，商品
     欄位維持空白，讓使用者自己從曝光資源比對後手動上傳，不會自動代入
     main商品。 */
...
function removeMsbnVersion(instanceId){
  if(msbnVersionNumberFromId(instanceId) === 1){
