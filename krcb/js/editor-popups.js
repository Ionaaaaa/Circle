...
function runImport(excelFile, folderFiles){
  ...
  function processOneBlock(parsed, done){
    ...
    /* 2026-08(KRCB)：只有一組文案，POPUP不再有自己獨立的曝品表/LOGO2——
     直接用完整的folderFiles做主要商品比對，不用像海外購物節那邊特別
     保留檔案給popup用。 */
    ...
    /* 2026-08(KRCB)：使用者確認KRCB只有一組文案，不用區分POPUP——popup
       現在跟其他版位一樣共用host/logo2，不用再跳出第二次「popup商品/
       LOGO2」確認流程，這裡直接呼叫done()放行，不做任何事。 */
...
function openLayoutTogglePopup(){
  ...
  body.innerHTML = LAYOUT_REGISTRY.filter(function(l){ return !aliasBase[l.id] && !isMsbnTabOnlyId(l.id); }).map(function(l){
