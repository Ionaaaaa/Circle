...
var LAYOUT_REGISTRY = [
  ...
  /* 2026-08(KRCB)新增：副區——固定排在msbn分頁最前面的第0格(見
     js/editor-state.js的buildMsbnTabData())，APP/PC兩個版型內容一樣、
     只有畫布尺寸不同，跟LPBN_APP/LPBN_PC同一個道理分兩個獨立版位。
...
/* ══════════════════ 背景圖版本清單(A/B/C...) ══════════════════
   2026-08(KRCB專案)修正：一開始設計成「每個版位各自獨立選版本」
   (S.bgVersion[layoutId])，但使用者確認HBN/DD/LPBN/MSBN這些不同版位的
   「A版」講的是同一件事(同一批活動素材/同一季的設計)，不是各版位各自
   獨立切換——不會有「HBN已經是B版、但DD Card還停在A版」這種情況，全部
   版位永遠一起换。
   改成PROJECT_BG_VERSION這一個全域字串，不分版位、也不分頁(MSBN是獨立
   的專屬分頁，但版本一樣要跟main分頁的HBN/DD/LPBN同步，不能用「跟著
   分頁走」的存法，所以刻意不放進S/tab.data，而是放在分頁系統之外的
   全域變數，切分頁不會受影響，見js/editor-export.js/js/editor-main.js
   暫存檔存檔/讀檔那兩處會額外存讀這個值)。
...
/* 2026-08(KRCB)新增：副區(subzone_app/subzone_pc)固定排在msbn分頁最前面
   的第0格，不是「MSBN1,2,3...」這種可以刪除/重新編號的動態版本，所以
   刻意不算進MSBN_BASE_IDS/isMsbnFamilyId()——那個函式專門管「MSBN家族
   版型」的版本編號/下載檔名邏輯(msbn1.jpg、msbn2.jpg...)，副區的下載
   檔名要比照LPBN_APP/PC那種一般命名規則，不是msbnN.jpg這種。
   但副區在畫布互動(拖曳/縮放/選取icon，見js/msbn-logo-interaction.js)、
   文字合規檢查(js/msbn-text-compliance.js)、msbn分頁排他(不會出現在
   一般分頁的追加版位清單裡)這幾件事，行為要跟MSBN完全一樣——這幾個地方
   改用這支涵蓋範圍更大的判斷式，不要用isMsbnFamilyId()。 */
var SUBZONE_IDS = ['subzone_app', 'subzone_pc'];
function isMsbnTabOnlyId(layoutId){
  return isMsbnFamilyId(layoutId) || SUBZONE_IDS.indexOf(layoutId) !== -1;
}
...
var S = {
  ...
  /* ══════════════════ 背景圖版本(A/B/C...) ══════════════════
  /* ══════════════════ 背景圖版本(A/B/C...) ══════════════════
     2026-08(KRCB專案)：背景版本是「整個專案共用一個」，不是per-layout/
     per-tab各自獨立——見上面PROJECT_BG_VERSION/getBgVersion()/
     setBgVersion()的說明，這裡S物件不需要放任何欄位，version這個概念
     完全不經過S/tab.data，切分頁、匯入/讀取暫存檔都不會動到它。 */
  /* ── 文案分組（媽咪會員案新增） ──
...
function newEmptyTabData(label){
  return {
    ...
    /* 2026-08(KRCB)新增：MSBN商品格子(host/host1/host2)的「多素材合成」
       編輯狀態(貼地陰影1200畫布，見js/shadow-system/shadow-popup.js的
       openMsbnHostComposePopup())——key是"instanceId::slotKey"組合字串，
       每一格各自獨立記住自己的shadowSlots/shadowCombo等完整合成狀態，
       下次點開同一格繼續調整時可以還原，不用每次都重新上傳。跟main商品
       自己的S.shadowSlots是完全分開的兩份資料，互不影響。 */
    msbnShadowSlots: {},
    arVariant: 'activity',
    logo2Raw: null,
    logo2Scale: 1,
...
function buildMsbnTabData(label, count){
  ...
  data.activeLayoutIds = ['07_msbn'].concat(SUBZONE_IDS);
  var instances = [];
  /* 2026-08(KRCB)新增：副區固定排最前面的第0格，APP/PC兩個版型都給，
     不像MSBN1~N是動態可新增/刪除的版本——這兩筆直接寫死instanceId=
     layoutId(跟本尊'07_msbn'的模式一樣)，不會被Excel「版型」欄位覆寫、
     也不會出現在「＋新增MSBN版本」的版型選擇popup裡(那個popup只列
     07_msbn開頭的版型，見js/editor-main.js的populateMsbnAddInlineSelect/
     openAddMsbnVersionPopup相關程式碼，SUBZONE_IDS不在那個過濾條件裡)。 */
  SUBZONE_IDS.forEach(function(id){
    instances.push({ instanceId: id, layoutId: id, label: null, textGroup: '文案1', slotNames: null });
  });
...
function saveCurrentTabIntoData(){
  ...
  tab.data.msbnShadowSlots = JSON.parse(JSON.stringify(S.msbnShadowSlots || {}));
}
...
function applyTabData(i, cb){
  ...
  S.msbnShadowSlots = JSON.parse(JSON.stringify(d.msbnShadowSlots || {}));
