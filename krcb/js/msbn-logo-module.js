...
window.Modules.msbnLogoSlot = {
  draw: function(ctx, layer, state, layoutMeta){
    ...
    /* 2026-08(KRCB)修正：這個提示圖示(點擊可開合成工具/圖庫)已經搬到
       js/msbn-logo-interaction.js的drawMsbnLogoOverlay()裡畫——那裡只有
       互動編輯的即時預覽(renderAll())會呼叫到，匯出/下載前的乾淨版本
       (renderLayoutClean())不會呼叫，這樣提示圖示才不會被一起烤進下載
       的圖片檔案裡。這裡(Core.render()的一部分，下載時也會執行到)不再
       畫這個提示圖示。 */
