...
(function(){
  ...
  /* 2026-08再調整(蝦皮流行穿搭案)：舞台放大了(js/shadow-system/
     ...
     2026-08再調整：舞台再放大10%(wPct 0.75→0.825)，這裡同步再放大10%
     (0.4638→0.5102)。 */
  var PRODUCT_BASE_HPCT   = 0.5102; // 2026-08再調整：跟著舞台再放大10%，從0.4638放大到0.5102，維持商品相對舞台的比例不變。
  ...
  var PRODUCT_POS_CENTER = { xPct: 0.5,  yPct: 0.533333, hPct: PRODUCT_BASE_HPCT }; // 三個裡最大、最主要，剛好在畫布正中央。2026-08再調整：上次只往上移30px使用者反映幾乎沒感覺、還是比右邊商品低，這次直接大幅往上調(yPct 0.716667→0.55)，確保明顯比商品3(右，yPct 0.64)高。2026-08再調整：全部商品位置整體往上移20px(yPct 0.55→0.533333)。
  var PRODUCT_POS_RIGHT  = { xPct: 0.78, yPct: 0.623333, hPct: PRODUCT_BASE_HPCT }; // 2026-08再調整：再往上一點（原本0.62還是偏低），跟左/中兩個的底部差距拉大一些。2026-08再調整：往右10px(xPct 0.72→0.728333)、往下10px(yPct 0.631667→0.64)，錨點依然是底部置中，不是換算法。2026-08再調整：再往右一點(xPct 0.728333→0.78)，跟人物1(headXPct改成0.20，往左移)拉開更多距離。2026-08再調整：全部商品位置整體往上移20px(yPct 0.64→0.623333)。
  var PRODUCT_POS_LEFT   = { xPct: 0.26, yPct: 0.725, hPct: PRODUCT_BASE_HPCT }; // 2026-08再調整：全部商品位置整體往上移20px(yPct 0.741667→0.725)。
