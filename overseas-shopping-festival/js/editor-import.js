...
var MSBN_TEMPLATE_SLOT_MAP = {
  '版型1': {
    layoutId: '07_msbn',
    groups: {
      '左': { 'LOGO': {key:'left', type:'logo'} },
      '中': { 'LOGO': {key:'mid', type:'logo'} },
      '右': { 'LOGO': {key:'right', type:'logo'} }
    }
  },
  '版型2': {
    layoutId: '07_msbn_v2',
  ...
  '版型5': {
    ...
    groups: {
      ...
      '右': { '大字(10字內)': {key:'textBig', type:'text'}, '小字(10字內)': {key:'textSmall', type:'text'} }
    }
  },
  '版型6': {
    layoutId: '07_msbn_v6',
    groups: {
      '左': { 'LOGO': {key:'logo1', type:'logo'} },
      '右': { '大字(10字內)': {key:'brandName', type:'text'}, '小字(10字內)': {key:'discount', type:'text'} }
    }
  }
};
      ...
      anchors.forEach(function(anchor, idx){
        ...
        for(var rr=r+1; rr<=blockEnd; rr++){
          ...
          for(var cc=colStart; cc<colEnd && cc<rowX.length; cc++){
            ...
            var label = cell.trim();
            var mapping = fieldMap[label];
            if(!mapping) continue;

            var value;
            if(label.indexOf('(10字內)') >= 0){
              /* 「大字(10字內)」「小字(10字內)」這種是inline排法：名稱跟值
                 同一列、值在名稱右邊一欄。 */
              value = rowX[cc+1];
            } else {
              /* LOGO/文案/圖片是vertical排法：名稱單獨一列，值在正下方
                 那一列、同一欄。 */
            ...
            if(mapping.type === 'logo') logos[mapping.key] = value;
            else texts[mapping.key] = value;
          }
        }
      });
      result[version] = { layoutId: tmpl.layoutId, logos: logos, texts: texts };
    }
