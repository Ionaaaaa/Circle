...
function applyMsbnAssetsFromImport(slotNames, folderFiles){
  ...
  instances.forEach(function(inst){
    ...
    // ② LOGO/圖片類欄位 → S.msbnLogos（沿用原本的圖片載入機制，slotKey改成動態的）
    Object.keys(info.logos || {}).forEach(function(slotKey){
      var name = info.logos[slotKey];
      if(!name) return;
    ...
    // ③ 文案類欄位 → S.msbnTexts（純字串，不用像圖片那樣非同步載入，直接寫入即可）
    Object.keys(info.texts || {}).forEach(function(slotKey){
      var content = info.texts[slotKey];
      if(!content) return;
      if(isActive){
        S.msbnTexts = S.msbnTexts || {};
        S.msbnTexts[inst.instanceId] = S.msbnTexts[inst.instanceId] || {};
        S.msbnTexts[inst.instanceId][slotKey] = content;
      } else {
        var tabData2 = TABS[tabIndex].data;
        tabData2.msbnTexts = tabData2.msbnTexts || {};
        tabData2.msbnTexts[inst.instanceId] = tabData2.msbnTexts[inst.instanceId] || {};
        tabData2.msbnTexts[inst.instanceId][slotKey] = content;
      }
    });
  });
