...
function runImport(excelFile, folderFiles){
  ...
  /* ══════════════════ 共用商品快照 ══════════════════
     ...
     商品/人物只需要使用者在1200畫布排一次，後面標記為共用的區塊直接
  ...
  function processOneBlock(parsed, done){
    ...
    /* 2026-08修正「main商品會吃到popup的商品」：matchAssetFolder()裡有一段
       通用備援(SLOT_ALIASES猜測)，Excel曝品表沒填名稱的槽位(例如main的
       「商品2(中)」這次剛好留空)，會拿「目前還沒被別的槽位用掉」的圖檔
       去猜——問題是popup專屬的曝品表(parsed.popupExposure)這時候還沒
       輪到它比對，它保留給popup用的檔案在main比對的當下都還算「沒被用
       掉」，於是main的備援猜測就把popup的檔案搶走了。
       修法：main比對之前，先把popup曝品表明確指定要用的檔名從
       folderFiles裡「保留」起來，不讓main看到這些檔案，備援猜測自然
       就不會猜到它們身上；popup自己稍後比對時，用的是完整的folderFiles
       (不受這個保留影響)，一樣可以正常吃到自己該用的檔案。 */
    var popupExposure = parsed && parsed.popupExposure;
    var reservedForPopupNames = (popupExposure && popupExposure.items || [])
      .map(function(item){ return item.name; })
      .filter(Boolean);
    if(parsed && parsed.popupLogo2Checked && parsed.popupLogo2MaterialName){
      reservedForPopupNames.push(parsed.popupLogo2MaterialName);
    }
    var mainFolderFiles = (folderFiles && reservedForPopupNames.length)
      ? folderFiles.filter(function(f){
          return !reservedForPopupNames.some(function(name){ return matchFileByAliases([f], [name]); });
        })
      : folderFiles;
    var matched = (mainFolderFiles && mainFolderFiles.length)
      ? matchAssetFolder(mainFolderFiles, exposure && exposure.items, extraAliases)
      : {};
    ...
    /* 2026-08新增：確認完main商品之後，如果這個區塊有popup專屬的曝品表
       (parsed.popupExposure，見editor-import.js的parsePopupExposureTable())，
       接著自動跳出「同一個確認彈窗」第二次，這次比對/顯示的是popup自己的
       素材、確認完寫進S.assets.popupHost（不是S.assets.host），兩邊完全
       獨立，見js/shadow-system/shadow-popup.js的openShadowPopup()第二個
       參數說明。使用者流程：main商品確認一次 → popup商品確認一次 → 兩個
       都確認完才算這個區塊處理完畢、繼續下一個區塊。
       2026-08再新增：popup(文案2)如果自己也有勾LOGO2(見
       editor-import.js的parsePopupLogo2Info())，要在popup商品確認「之前」
       先跳出LOGO2確認popup——順序照main那邊的既有習慣(main也是logo2先
       確認完才進商品確認)，寫進S.assets.popupLogo2，不是共用的
       S.assets.logo2。 */
    function goToPopupShadowStepThenDone(){
      var popupExposure = parsed && parsed.popupExposure;

      function goToPopupProductStep(){
        if(!popupExposure || !popupExposure.items || !popupExposure.items.length){
          done();
          return;
        }
      ...
      var popupLogo2Checked = !!(parsed && parsed.popupLogo2Checked);
      if(!popupLogo2Checked){
        goToPopupProductStep();
        return;
      }

      var popupLogo2Name = parsed && parsed.popupLogo2MaterialName;
      var popupImageFiles = (folderFiles || []).filter(function(f){ return /\.(png|jpe?g|webp)$/i.test(f.name); });
      var popupLogo2File = popupLogo2Name ? matchFileByAliases(popupImageFiles, [popupLogo2Name]) : null;

      if(popupLogo2File){
        logo2AutoApplyFromFile(popupLogo2File, function(err){
          if(err) console.warn('[editor-popups] popup LOGO2自動套用失敗：', err);
          renderAll();
          openLogo2Editor(goToPopupProductStep, 'popupLogo2');
        }, 'popupLogo2');
      } else {
  ...
  function afterParsed(parsedResult){
    /* 不管有沒有blocks(可能整份Excel完全沒有一般版位、只有MSBN那幾個分頁)，
       只要有讀到MSBN版型列表分頁的資料，就先把品牌/LOGO素材名稱拿去跟
...
/* 資料夾裡比對到的人物/商品檔案，不再用「靜態預覽+廣播位置」這套（那套只適合
   ...
   組合怎麼決定，優先順序：
     ...
     2. 猜不到才退回 guessComboFromMatchedSlots()，依照資料夾實際比對到哪些
        槽位反推一個最合理的組合 */
function proceedToShadowFromImport(matched, exposure, onConfirm, targetAssetKey){
  /* 2026-08新增：targetAssetKey不是'host'(例如'popupHost')時，要先把popup
     自己獨立那組合成狀態換進來，再開始設定S.shadowSlots/S.shadowCombo——
     順序很重要，要先換好底再蓋上這次從Excel/資料夾比對到的內容，不然會
     被隨後的_swapInPopupShadowState()覆蓋掉剛設定好的值。 */
  if(targetAssetKey && targetAssetKey !== 'host') _swapInPopupShadowState();
