...
/* 把目前的LOGO2狀態（S.logo2Scale/OffX/OffY/Shape/BgColor + _logo2Img）
   ...
   同一份合成邏輯，結果保證一致。 */
function logo2Composite(cb, targetAssetKey){
  if(!_logo2Img){
  ...
  function finishWithDataUrl(dataUrl){
    ...
    img.onload = function(){
      S.assets = S.assets || {};
      S.assets[targetAssetKey || 'logo2'] = img;
      if(cb) cb();
    };
...
function logo2AutoApplyFromFile(file, cb, targetAssetKey){
  if(targetAssetKey && targetAssetKey !== 'logo2') _swapInPopupLogo2State();
  var reader = new FileReader();
  reader.onload = function(ev){
    logo2LoadFromDataUrl(ev.target.result, function(err){
      if(err){ if(cb) cb(err); return; }
      logo2Composite(function(err2){
        if(targetAssetKey && targetAssetKey !== 'logo2') _swapOutPopupLogo2State();
        if(cb) cb(err2);
      }, targetAssetKey);
    });
  };
...
/* 2026-08新增：popup版位需要自己獨立的LOGO2(popupLogo2)，不能跟main那組
   共用S.logo2Raw/Scale/OffX/OffY/Shape/BgColor/FillMode這幾個全域欄位——
   做法跟js/shadow-system/shadow-popup.js處理popupHost是同一個套路：
   進編輯popup前先把目前這份(main的)存到旁邊、換上popup自己那份給使用者
   編輯；離開時(合成完成後)存回popup自己的欄位、再把main那份還原回來。
   對這支檔案裡其餘所有函式(logo2InitFit、drawLogo2Canvas、拖曳/縮放
   互動...)來說完全無感，它們永遠只讀寫S.logo2Xxx這幾個欄位本身，不用
   額外傳參數、不用改一行——這幾個欄位「當下究竟是main的還是popup的」
   由這裡的swap函式決定。 */
var _logo2TargetAssetKey = 'logo2';
var _logo2SwappedOut = null;
var _logo2ImgSwappedOut = null;
...
function _swapInPopupLogo2State(){
  ...
  S.logo2Scale = S.popupLogo2Scale;
  S.logo2OffX = S.popupLogo2OffX || 0;
  S.logo2OffY = S.popupLogo2OffY || 0;
...
function _swapOutPopupLogo2State(){
  S.popupLogo2Raw = S.logo2Raw;
  S.popupLogo2Scale = S.logo2Scale;
  S.popupLogo2OffX = S.logo2OffX;
  S.popupLogo2OffY = S.logo2OffY;
  ...
  if(_logo2SwappedOut){
    ...
    S.logo2OffX = _logo2SwappedOut.logo2OffX;
    S.logo2OffY = _logo2SwappedOut.logo2OffY;
...
/* ── popup UI ──
   ...
   targetAssetKey(選填，預設'logo2')：這次編輯完，合成結果要寫進
   S.assets的哪個key。傳'popupLogo2'時會自動切換成popup自己獨立那組
   LOGO2狀態，使用者在編輯popup裡看到/調整的是popup自己的LOGO2。 */
function openLogo2Editor(onDone, targetAssetKey){
  _logo2TargetAssetKey = targetAssetKey || 'logo2';
  if(_logo2TargetAssetKey !== 'logo2') _swapInPopupLogo2State();
  var hasExisting = !!(S.assets && S.assets[_logo2TargetAssetKey]);
  ...
  if(removeBtn){
    removeBtn.onclick = function(){
      S.assets = S.assets || {};
      S.assets[_logo2TargetAssetKey] = null;
