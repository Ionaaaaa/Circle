...
(function(){
  ...
  var PRODUCT_POS_CENTER = { xPct: 0.5,  yPct: 0.741667, hPct: PRODUCT_BASE_HPCT }; // 三個裡最大、最主要，剛好在畫布正中央
  var PRODUCT_POS_RIGHT  = { xPct: 0.72, yPct: 0.631667, hPct: PRODUCT_BASE_HPCT }; // 2026-08再調整：再往上一點（原本0.62還是偏低），跟左/中兩個的底部差距拉大一些
  ...
  window.ShadowLayoutDefaults = {
    ...
    // getShadowOrder()）：商品1(左)最下層、商品3(右)第二層、商品2(中)最上層。
    D: {
  ...
  /* 每個內部版型代號，實際會用到哪些槽位（給素材清單UI知道要顯示哪幾個上傳欄位）。
     ...
     陣列前面＝後方、後面＝前方）——商品1(左)排最前面＝最下層，商品3(右)第二＝中間層，
     商品2(中)排最後面＝最上層，符合「中間商品預設在圖層最上方」的需求。使用者在
     popup裡自己拖曳清單調整過順序之後，就照使用者調整的結果，不會再被這裡的預設值
     蓋掉（見getShadowOrder()裡的sameSet判斷）。 */
