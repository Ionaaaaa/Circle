...
function ensureDynamicLayoutRegistered(instanceId, layoutId, label){
  ...
  if(!base) return;
  var entry = { id: instanceId, name: label || base.name, configFile: base.configFile };
  ...
  /* 蝦皮家居-寢具新增需求：MSBN這個版位「版型很多種、版本會一直往下加」
     ...
     其他文字或編號前綴）。這裡直接照instanceId本身的編號規則(見
     msbnVersionNumberFromId())算出name/exportName，不用每個呼叫端各自
     命名，行為才會一致——不管是Excel匯入自動產生的實例、還是使用者在
     工具裡按「＋新增MSBN版本」手動加的實例，都會套用同一套命名。 */
  if(layoutId === '07_msbn'){
  ...
  LAYOUT_REGISTRY.push(entry);
  window.LAYOUT_ALIAS_BASE[instanceId] = layoutId;
}

/* MSBN複製實例的id格式固定是'07_msbn'(第1版)、'07_msbn__2'、'07_msbn__3'...
   —— 從id反推「這是第幾版」，給檔名/顯示名稱統一使用，只需要維護這一處。 */
function msbnVersionNumberFromId(instanceId){
  var m = /__(\d+)$/.exec(instanceId);
  return m ? parseInt(m[1], 10) : 1;
}

/* 是否為MSBN家族的版位id（本尊'07_msbn'或它動態複製出來的任何實例）——
   下載檔名規則、新增/刪除版本按鈕都要用同一個判斷式，集中寫在這裡避免
   各處各寫一次、以後改判斷邏輯漏改。 */
function isMsbnFamilyId(layoutId){
  if(layoutId === '07_msbn') return true;
  var aliasBase = window.LAYOUT_ALIAS_BASE || {};
  return aliasBase[layoutId] === '07_msbn';
}
...
/* 這個專案要求：MSBN固定只出現在專屬的「msbn」那個分頁，一般分頁(第一
   ...
   預設清單要排除'07_msbn'，跟msbn那個分頁分開管理(見下面
   buildMsbnTabData()會自己覆寫成只有'07_msbn')。兩處地方(S的初始值、
   newEmptyTabData())都要用這份清單，不要各自重複寫一次filter。 */
var NON_MSBN_LAYOUT_IDS = LAYOUT_REGISTRY.filter(function(l){ return l.id !== '07_msbn' && !l.defaultOff; }).map(function(l){ return l.id; });
...
var S = {
  ...
  layoutTextGroup: {},
  activeTextGroup: '文案1',
  assets: { logo1:null, logo2:null, host:null, ctaDD:null, ctaGo:null, ctaGame:null, arActivityLogo:null },
...
function buildMsbnTabData(label, count){
  ...
  for(var i=1; i<=n; i++){
    var instanceId = (i === 1) ? '07_msbn' : ('07_msbn__' + i);
...
function applyTabData(i, cb){
  ...
  S.assets = { logo1:null, logo2:null, host:null, ctaDD:null, ctaGo:null, ctaGame:null, arActivityLogo:null };
