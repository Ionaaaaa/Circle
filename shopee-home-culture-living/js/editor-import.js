...
function parseRows(rows){
  ...
  /* AR內容/文案這兩格在範本裡永遠存在，不管這次工單有沒有勾選AR都會被
     parseARCell()讀到值——但「製作素材」欄位（左邊）才是真正決定這次
     工單要不要AR版位的依據，沒列在那邊的一律不需要出現，包含這裡讀到的
     AR文案/版本。所以這裡要跟filterLayoutsByMaterials()同一個判斷基準
     （用同一份LAYOUT_MATERIAL_KEYWORDS.ar關鍵字）反查一次，materials
     裡沒有命中「AR」關鍵字，就把讀到的arInfo整個丟掉，當作沒有這回事。 */
  var arSelected = materials.some(function(m){
    return _keywordHit(String(m||'').toUpperCase(), 'AR');
  });
  if(!arSelected) arInfo = null;
