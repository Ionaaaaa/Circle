  ...
  var loaders = activeLayouts().map(function(layout){
    ...
    block.innerHTML =
      '<div class="canvas-meta">'+
        ...
        '<button class="mini-dl-btn" onclick="triggerCustomBgUpload(\''+layout.id+'\')">'+ICON_IMAGE+' 上傳背景圖</button>'+
        (S.customBg && S.customBg[layout.id] ?
          '<button class="mini-dl-btn" style="color:#b94a3d;" onclick="clearCustomBg(\''+layout.id+'\')">✕ 清除背景圖</button>' : '')+
...
function addTabFromImport(parsed){
  ...
  ['標題','副標','日期','AR文案'].forEach(function(k){ if(parsed.text[k]) data.text[k] = normalizeTextValue(k, parsed.text[k]); });
