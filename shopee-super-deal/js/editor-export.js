...
/* 有K數上限規定的版位，用JPEG格式輸出＋自動壓縮找到符合大小上限、畫質
   盡量高的quality；數字直接取自蝦皮超划算「站內BN」清單的K數欄位。
   注意：05_ddcard／08_coin_bn這兩個雖然沿用上一個蝦皮媽咪會員日案子的
   版面結構，但K數限制改用這次蝦皮超划算工單自己的數字(245K/<150KB)，
   不是照搬舊案子當時的145——K數限制是工單自己訂的規格，不是版面結構
   的一部分，兩個專案可能不一樣。
   2026-09：skinny_pc不在這個表裡——使用者確認Skinny BN PC要輸出PNG，
   只有Skinny BN APP要輸出JPG，兩個雖然是同一個Skinny BN系列，但下載
   格式不一樣，不能套同一條規則。 */
var EXPORT_SIZE_LIMIT_KB = {
  '03_c2c_bn': 245,
  'skinny_app': 150,
  '05_ddcard': 245,
  '05_ddcard_nologo': 245,
  '08_coin_bn': 150,
  '04_ig': 450
};

/* 這幾個版位工單K數欄填'X'(=沒有明確K數上限規定，見上面表格的說明)，
   但規格仍要求JPG格式(不是PNG)，所以維持JPEG輸出、只是不用像有K數上限
   的版位那樣反覆試壓縮找臨界點，固定用高畫質(0.92)出圖即可。11_lpbn_app／
   12_lpbn_pc是沿用上一個案子就有的規則；07_msbn／ar／10_game_bn目前
   維持退回下面的PNG無損輸出(沿用舊案子的行為，這幾個版位工單沒有明確
   要求一定要JPG)。 */
var EXPORT_JPEG_NO_LIMIT = { '11_lpbn_app': true, '12_lpbn_pc': true };
var EXPORT_JPEG_FIXED_QUALITY = 0.92;

/* SKBN(Skinny BN APP/PC)的下載檔名是backgrounds/README.txt明講的固定格式
   (例如'07_Skinny_BN_APP_2'，數字對應版頭商品1(左)/2(中)/3(右))，不是
   downloadSingle()/downloadAll()原本那套「流水號(照畫布顯示順序)_版位
   名稱」的通用命名可以套出來的——這裡直接列一份對照表，查得到就直接用
   這個檔名(不再加流水號前綴，檔名本身已經帶了_1/_2/_3，加了反而變成
   '3_07_Skinny_BN_APP_2'這種奇怪的雙重編號)，查不到(其他所有版位)才
   退回原本的通用命名，兩者不衝突、不影響其他版位。 */
var SKBN_DOWNLOAD_FILENAME = {
  'skinny_app':    '07_Skinny_BN_APP_1',
  'skinny_app__2': '07_Skinny_BN_APP_2',
  'skinny_app__3': '07_Skinny_BN_APP_3',
  'skinny_pc':     '07_Skinny_BN_PC_1',
  'skinny_pc__2':  '07_Skinny_BN_PC_2',
  'skinny_pc__3':  '07_Skinny_BN_PC_3'
};

/* downloadSingle()／downloadAll()共用的檔名(不含副檔名)決定邏輯——
   SKBN查得到固定檔名就直接用；查不到就退回原本「流水號_版位名稱」的
   通用命名(idx可能是null，例如這個版位不在目前顯示順序裡，這時候
   sanitizeFilename()前面不加流水號，行為跟原本downloadSingle()一致)。 */
function downloadFileBaseName(layoutId, idx, layoutName){
  if(SKBN_DOWNLOAD_FILENAME[layoutId]) return SKBN_DOWNLOAD_FILENAME[layoutId];
  var prefix = idx ? (idx+'_') : '';
  return prefix + sanitizeFilename(layoutName);
}

/* 「Me Page Circle」「Me Page Circle_new」這兩個版位，編輯器裡只做一個
   大尺寸(192x192)版本，下載時額外自動縮小輸出一份小尺寸(64x64)——工單
   把這兩種尺寸各自列成獨立的製作物名稱(「09_Me Page Circle」大版／
   「10_Loyalty Page icon」小版)，但視覺內容完全相同，不需要使用者自己
   再調一次縮小版，下載時順便一起產生即可。 */
var ME_PAGE_DUAL_EXPORT = {
  '09_me_page_circle':     { largeName:'09_Me_Page_Circle',     smallName:'10_Loyalty_Page_icon',     smallSize:64 },
  '09_me_page_circle_new': { largeName:'09_Me_Page_Circle_new', smallName:'10_Loyalty_Page_icon_new', smallSize:64 }
};

/* 把來源canvas等比縮小畫到一個新的size x size正方形canvas上，回傳新
   canvas——給ME_PAGE_DUAL_EXPORT用。來源本來就是192x192正方形，縮小成
   64x64一樣是正方形，不用處理長寬比不同的情況；imageSmoothingQuality
   設'high'，縮小圖示類的簡單圖形這樣就夠銳利，不會糊。 */
function _shrinkCanvasSquare(srcCanvas, size){
...
/* 2026-09新增：外廣(waiguang)大部分尺寸下載都要JPG，只有02_Facebook_DPA
   這個大項比較特別——它的背景美術本身就是PNG(帶透明或需要無損)，下載也
   要維持PNG，不能跟其他外廣一樣統一轉JPG。這裡列的是「大項key」，不是
   完整layoutId(因為同一個大項底下有好幾個尺寸，每個尺寸都要套用同一條
   規則，直接比對大項比對到完整layoutId清單簡單)。 */
var EXPORT_PNG_OVERRIDE_WAIGUANG_SECTIONS = { '02_Facebook_DPA': true };

/* 判斷是不是外廣(waiguang)的layoutId——跟modules/background-module.js的
   parseWaiguangLayoutId()同一套判斷方式(id格式'{大項key}__{尺寸}'，尺寸
   是'數字x數字'這種格式)，這裡沒有共用那支函式(不同檔案、沒有互相
   require)，重新寫一份一樣邏輯的判斷，抓到大項key。 */
function _waiguangSectionOf(layoutId){
...
/* 依版位決定輸出格式：有K數上限的版位→JPEG+自動壓縮；LPBN_APP/PC→固定
   高畫質JPEG（不用試壓縮）；外廣(waiguang)預設也是固定高畫質JPEG，除非
   在EXPORT_PNG_OVERRIDE_WAIGUANG_SECTIONS裡登記；其他(站內非以上情況)→
   PNG無損。回傳 {blob, ext}，呼叫端用ext決定副檔名，不用另外判斷一次格式。 */
function exportLayoutBlob(layoutId, canvas){
  ...
  var wgSection = _waiguangSectionOf(layoutId);
  if(wgSection && !EXPORT_PNG_OVERRIDE_WAIGUANG_SECTIONS[wgSection]){
    return canvasToBlobJPEG(canvas, EXPORT_JPEG_FIXED_QUALITY).then(function(blob){
      return { blob: blob, ext: 'jpg' };
    });
  }
...
function downloadSingle(layoutId){
  ...
  var baseName = downloadFileBaseName(layoutId, idx, namePart);
  var dual = ME_PAGE_DUAL_EXPORT[layoutId];
  confirmDownloadWithComplianceCheck(function(){
    ...
    exportLayoutBlob(layoutId, canvas).then(function(res){
      triggerDownload(res.blob, (dual ? dual.largeName : baseName)+'.'+res.ext);
      if(!dual){ renderAll(); return; }
      /* 額外自動輸出一份縮小版(見ME_PAGE_DUAL_EXPORT的說明)，格式規則
         (PNG/JPEG)沿用同一個layoutId的exportLayoutBlob()判斷，不用另外
         寫一套。 */
      var small = _shrinkCanvasSquare(canvas, dual.smallSize);
      exportLayoutBlob(layoutId, small).then(function(res2){
        triggerDownload(res2.blob, dual.smallName+'.'+res2.ext);
        renderAll();
      });
    });
  });
        ...
        buildCanvasArea().then(function(){
          applyDefaultLogos(function(){
            ...
            var jobs = order.map(function(layout, idx){
              ...
              var dual = ME_PAGE_DUAL_EXPORT[layout.id];
              return exportLayoutBlob(layout.id, canvas).then(function(res){
                folder.file((dual ? dual.largeName : downloadFileBaseName(layout.id, idx+1, layout.name))+'.'+res.ext, res.blob);
                if(!dual) return;
                var small = _shrinkCanvasSquare(canvas, dual.smallSize);
                return exportLayoutBlob(layout.id, small).then(function(res2){
                  folder.file(dual.smallName+'.'+res2.ext, res2.blob);
                });
              });
            });
    ...
    exportOneTab(0, function(){
      ...
      zip.generateAsync({type:'blob'}).then(function(blob){
        /* zip檔本身的檔名維持原本邏輯：使用者按下「整包下載」當下所在的
           ...
           2026-09新增：外廣(waiguang)分頁優先用tab.data.waiguangOrderName
           (Excel工單的「工作項目名稱」)當檔名，不是分頁標籤(分頁標籤是
           大項名稱，例如'01_LINE LAP'，不是使用者真正想要的案名)——站內
           分頁沒有這個欄位(維持null)，退回原本用分頁標籤的邏輯，不受影響。 */
        var tab = TABS[originalActiveTab];
        var tabName = tab ? (tab.data.waiguangOrderName || tab.data.label) : '未命名工單';
