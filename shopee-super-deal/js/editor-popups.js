...
/* ══════════════════ 1. 匯入工單彈窗（Excel + 資料夾 雙上傳） ══════════════════
   2026-09改版：站內／站外選擇不再是獨立的一個popup，改成這個彈窗上方的
   頁籤——每次「全新開始」(DOMContentLoaded自動跳出的那次)都會回到預設
   頁籤(站內)，使用者可以在裡面切換；「載入暫存」(bindLoadTemp)是完全
   不同的路徑，直接讀存檔裡的siteMode欄位、不會開這個彈窗，天生就符合
   「重新匯入暫存檔不用再選一次」的規則。
   2026-09再改版：站外版位/config/Excel解析器(js/editor-import-external.js
   的runExternalImport())已經接上了，這裡拿掉原本「還沒建置完成」的提示
   文字，改成跟站內一樣的拖曳上傳區(只需要Excel，素材資料夾外廣這一版
   還沒做LOGO/照片自動比對，先不顯示那個上傳區，避免使用者以為上傳了會
   有作用)。兩邊各自獨立一組_importState，互不影響，確認匯入時依目前
   S.siteMode決定要呼叫runImport()(站內)還是runExternalImport()(站外)。 */

var _importState = { excelFile:null, folderFiles:[] };
var _importStateExternal = { excelFile:null, folderFiles:[] };

function switchSiteModeTab(overlay, mode){
  S.siteMode = mode;
  overlay.querySelector('#tab-internal').classList.toggle('active', mode==='internal');
  overlay.querySelector('#tab-external').classList.toggle('active', mode==='external');
  var bodyInternal = overlay.querySelector('#import-body-internal');
  var bodyExternal = overlay.querySelector('#import-body-external');
  if(mode === 'internal'){
    bodyInternal.style.display = '';
    bodyExternal.style.display = 'none';
  } else {
    bodyInternal.style.display = 'none';
    bodyExternal.style.display = '';
  }
}

function openImportModal(){
  _importState = { excelFile:null, folderFiles:[] };
  _importStateExternal = { excelFile:null, folderFiles:[] };
  var initialMode = (S.siteMode === 'external') ? 'external' : 'internal';

  var overlay = createOverlay(
    '<div class="popup-panel" style="width:440px;">'+
      '<div class="popup-head">'+
        '<div style="display:flex;gap:6px;">'+
          '<button class="tbtn sitemode-tab" id="tab-internal">站內</button>'+
          '<button class="tbtn sitemode-tab" id="tab-external">站外</button>'+
        '</div>'+
        '<button class="popup-x" onclick="closePopup()">×</button>'+
      '</div>'+
      '<div class="popup-body">'+

        '<div id="import-body-internal">'+
        ...
        '<div id="import-body-external" style="display:none;">'+
          '<div id="zone-excel-ext" class="dropzone">'+
            '<div class="dropzone-icon">'+ICON_FILE+'</div>'+
            '<div id="zone-excel-ext-title">拖曳外廣 Excel 工單到這裡</div>'+
            '<div class="hint" style="margin-top:4px;">或點擊選擇檔案</div>'+
          '</div>'+
          '<div id="zone-folder-ext" class="dropzone" style="margin-top:14px;">'+
            '<div class="dropzone-icon">'+ICON_FOLDER+'</div>'+
            '<div id="zone-folder-ext-title">上傳素材資料夾（LOGO1／LOGO2＋商品照片，可選）</div>'+
            '<div class="hint" style="margin-top:4px;">依檔名對照工單裡的商品名稱/LOGO廠商名稱，點擊選擇資料夾</div>'+
          '</div>'+
          '<div class="hint" style="margin-top:14px;">'+
            '外廣工單裡每個大項(例如「01_LINE LAP」)確認匯入後會各自變成一個'+
            '分頁，一個大項裡有好幾種尺寸/好幾組素材的話，會依序跳出調整'+
            '商品/編輯券的確認畫面，一組一組確認過去。'+
          '</div>'+
        '</div>'+
  ...
  overlay.querySelector('#tab-internal').onclick = function(){ switchSiteModeTab(overlay, 'internal'); };
  overlay.querySelector('#tab-external').onclick = function(){ switchSiteModeTab(overlay, 'external'); };
  switchSiteModeTab(overlay, initialMode);
  ...
  /* 外廣(站外)專用的Excel/資料夾拖曳區——跟上面站內那組是完全獨立的DOM
     元素/state(_importStateExternal)，避免互相蓋掉彼此選好的檔案。
     資料夾比對邏輯(matchAssetFolder())直接沿用站內那一套，商品名稱/
     LOGO廠商名稱都是從Excel「曝品表」/「LOGO」欄位讀出來的，比對方式
     完全一樣，不用另外寫一份。 */
  var zoneExcelExt = overlay.querySelector('#zone-excel-ext');
  var zoneFolderExt = overlay.querySelector('#zone-folder-ext');
  var excelInputExt = document.getElementById('excel-import-ext');
  var folderInputExt = document.getElementById('folder-import-ext');
  zoneExcelExt.onclick = function(){ excelInputExt.click(); };
  zoneFolderExt.onclick = function(){ folderInputExt.click(); };
  bindDropzone(zoneExcelExt, function(file){
    _importStateExternal.excelFile = file;
    zoneExcelExt.querySelector('#zone-excel-ext-title').textContent = '已選擇：'+file.name;
    zoneExcelExt.classList.add('dropzone-success');
  });
  excelInputExt.onchange = function(){
    var f = excelInputExt.files[0];
    if(f){
      _importStateExternal.excelFile = f;
      zoneExcelExt.querySelector('#zone-excel-ext-title').textContent = '已選擇：'+f.name;
      zoneExcelExt.classList.add('dropzone-success');
    }
    excelInputExt.value = '';
  };
  folderInputExt.onchange = function(){
    var files = Array.prototype.slice.call(folderInputExt.files);
    if(files.length){
      _importStateExternal.folderFiles = files;
      var imgCount = files.filter(function(f){ return /\.(png|jpe?g|webp)$/i.test(f.name); }).length;
      zoneFolderExt.querySelector('#zone-folder-ext-title').textContent = '已讀取 '+imgCount+' 張素材圖片';
      zoneFolderExt.classList.add('dropzone-success');
    }
    folderInputExt.value = '';
  };
  bindFolderDropzone(zoneFolderExt, zoneFolderExt.querySelector('#zone-folder-ext-title'), function(files){
    _importStateExternal.folderFiles = files;
    zoneFolderExt.classList.add('dropzone-success');
  });
  ...
  overlay.querySelector('#confirm-import-btn').onclick = function(){
    ...
    /* 站外模式下，Excel還沒選就按確認匯入——直接提醒，不要往下跑
       (runExternalImport()拿到undefined的file會在importWaiguangExcel()
       裡的FileReader那一步噴例外，這裡先攔下來給清楚一點的提示)。 */
    if(S.siteMode === 'external' && !_importStateExternal.excelFile){
      alert('請先選擇外廣的Excel工單檔案。');
      return;
    }
    if(S.siteMode !== 'external' && !_importState.excelFile){
      alert('請先選擇Excel工單檔案。');
      return;
    }

    btn.disabled = true;
    /* ★ 不要在這裡直接closePopup()——Excel解析/後續每個大項的1200 popup
       確認鏈都是非同步的，如果現在就把popup關掉，從「匯入中」到「下一個
       popup真的開起來」這段空窗期，畫面上會完全沒有任何遮罩，任何人都
       可以正常點擊背景的其他東西——很可能就是這樣才會在匯入過程中不小心
       觸發到別的操作（例如背景被連續觸發呼叫applyDefaultLogos()，進而
       搞亂LOGO2的合成狀態）。改成先換成一個「匯入中」的loading畫面撐著，
       一路撐到下一個真正的popup開起來為止——createOverlay()本身就會在
       建立新popup前先closePopup()清掉舊的，這裡完全不用自己手動關，
       銜接起來不會有任何空窗期。 */
    createOverlay(
    ...
    if(S.siteMode === 'external'){
      runExternalImport(_importStateExternal.excelFile, _importStateExternal.folderFiles);
    } else {
...
function runImport(excelFile, folderFiles){
  ...
  function processOneBlock(parsed, done){
    ...
    /* 2026-09新增：把這次匯入判斷到的logo2Checked存進S，讓右側面板的
       「編輯LOGO2」按鈕可以據此決定要不要顯示(見js/editor-main.js的
       updateLogo2FieldVisibility())——沒有勾選LOGO2的工單，直接把這顆
       按鈕整個藏起來，不要讓使用者看到一顆點了也沒有作用的按鈕。 */
    S.logo2Checked = logo2Checked;

    /* 2026-09修正：以前不管這次工單是「商品」還是「優惠券」樣式，一律
       固定開商品的1200畫布(proceedToShadowFromImport)——券樣工單通常
       曝品表是空的，開出來的商品popup沒有任何東西可以確認，使用者會
       完全看不到KV_Coupon券圖，也不知道要去哪裡才能看到(其實要另外手動
       點右側「編輯券」按鈕才對，但流程從沒帶使用者去那邊)。
       改成先看S.exposureStyle(匯入時addTabFromImport()→applyTabData()
       已經同步設定好了，這裡讀到的一定是這次區塊真正的樣式)：券樣
       工單直接自動開openCouponPopup()(KV_Coupon券圖+錢幣+使用者剛匯入的
       金額文案都會直接顯示)，不用再走商品那一整套陰影/照片比對流程；
       商品工單維持原行為不變。 */
    function goToShadowStep(){
      renderAll();
      if(typeof updateLogo2FieldVisibility === 'function') updateLogo2FieldVisibility();
      if(S.exposureStyle === 'coupon'){
        openCouponPopup(done);
      } else {
        proceedToShadowFromImport(matched, exposure, done);
      }
    }
...
function proceedToShadowFromImport(matched, exposure, onConfirm){
  ...
  /* 2026-09新增：曝品表的原價/標價一直有解析出來(見editor-import.js的
     parseExposureTable())，但從來沒有接到S.priceTags(小標的狀態)——
     使用者匯入工單後，開「調整商品/主持人」1200畫布，小標checkbox是關
     的、原價/特價輸入框是空的，要自己重新打一次字，等於Excel裡的原價/
     標價欄位形同虛設。這裡直接依曝品表內容預先寫進S.priceTags，任一格
     有值就自動打開小標，使用者開popup時就已經看得到內容，只要調整位置
     就好。 */
  (exposure && exposure.items || []).forEach(function(item){
  ...
  matchedShadowKeys.forEach(function(slotId){
    ...
    reader.onload = function(ev){
      ...
      /* 2026-09新增：跟js/shadow-system/shadow-popup.js的
         applyShadowSlotDataUrl()同一個理由——商品照片一到位，SKBN單品
         popup就有素材可以自動就定位了，不用等使用者把接下來自動彈出的
         「編輯商品」1200畫布popup關掉/確認完才觸發。這條路徑(Excel匯入
         資料夾比對)是直接寫S.shadowSlots，沒有經過applyShadowSlotDataUrl()，
         要在這裡另外補一次，蓋不到的話Excel匯入進來的商品一樣會卡住不
         出現在SKBN畫布上。 */
      if(type === 'product' && typeof ensureSkbnAutoCompose === 'function') ensureSkbnAutoCompose();
      pending--;
      if(pending<=0){
