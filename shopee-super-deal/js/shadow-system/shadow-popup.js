...
/* ══════════════════ KV(1200畫布)兩個固定錢幣，烤進host合成圖 ══════════════════
   2026-09新增，2026-09定案：兩個固定的錢幣裝飾圖案，直接烤進這個1200
   畫布最終輸出的host合成圖裡，跟商品/陰影/小標一起轉出——不是另外疊加
   在03_c2c_bn外層畫布上的東西。目前只支援商品模式(S.exposureStyle==
   'product')：商品模式的host是靠calcArtZoneFit這類「等比縮放、保留
   長寬比」的邏輯貼進03_c2c_bn畫布，烤在裡面的錢幣會跟著商品一起等比
   縮放，不會變形。券樣模式(coupon)的host其實是完全另一條路(設計師外部
   做好的成品直接匯入、滿版鋪畫布，不是靠這個1200畫布合成出來的)，錢幣
   要怎麼放要等券樣(coupon)功能真正動工時一併設計，這裡先不處理。
   兩個錢幣固定都會出現(沒有開關)，圖案是兩張不同的固定素材(S.assets.
   coin1/coin2，不用使用者上傳，見js/editor-main.js的loadDefaultCoin1()/
   loadDefaultCoin2())，使用者可以在這個1200畫布裡直接拖曳移動/拖角落
   縮放，位置存在S.kvCoinSlots(1200畫布本身的像素座標，x/y是中心點)。 */
var KV_COIN_KEYS = ['coin1', 'coin2'];
var _kvCoinSelected = {};      // coinKey -> boolean
var _kvCoinInteraction = null; // 目前拖曳中的錢幣互動狀態

/* 錢幣的預設位置(S.kvCoinSlots裡還沒有紀錄時才會用到)——數字是隨手估的
   起始值，不是照設計稿算出來的精確座標，實際請直接在1200畫布裡拖曳/
   縮放調整，調整結果會存進S.kvCoinSlots，之後都會沿用調整過的位置。 */
var KV_COIN_DEFAULTS = {
  coin1: { x: 980, y: 180, h: 130 },  // 右上，較小
  coin2: { x: 200, y: 620, h: 170 }   // 左中，較大
};

function _kvCoinCfg(coinKey){
  S.kvCoinSlots = S.kvCoinSlots || {};
  if(!S.kvCoinSlots[coinKey]){
    var def = KV_COIN_DEFAULTS[coinKey];
    S.kvCoinSlots[coinKey] = { x: def.x, y: def.y, h: def.h };
  }
  return S.kvCoinSlots[coinKey];
}

/* 算出目前這個錢幣的畫面box——寬度永遠依圖片自己的長寬比重新算(不單獨
   存寬度)，避免圖片萬一換了長寬比不同的新檔案時走鐘變形，跟SKBN那邊
   換商品照片會變形的bug是同一個道理，這裡從設計上直接避開。 */
function _kvCoinBox(coinKey){
  var img = S.assets && S.assets[coinKey];
  if(!(img instanceof HTMLImageElement) || !img.complete || !img.naturalWidth) return null;
  var cfg = _kvCoinCfg(coinKey);
  var h = cfg.h;
...
function drawKvCoins(ctx){
  KV_COIN_KEYS.forEach(function(coinKey){
    var box = _kvCoinBox(coinKey);
    if(!box) return;
    ctx.drawImage(box.img, box.x, box.y, box.w, box.h);
  });
}

function drawKvCoinSelection(ctx){
  KV_COIN_KEYS.forEach(function(coinKey){
    if(!_kvCoinSelected[coinKey]) return;
    var box = _kvCoinBox(coinKey);
    if(!box) return;
    ctx.save();
    ctx.strokeStyle = '#f59e0b'; // 用琥珀色跟商品的綠色選取框、小標的顏色區分
    ctx.lineWidth = 3;
...
/* 一定要在_shadowReceiver.attachPointerEvents(...)之前綁，同一個canvas上
   先註冊的監聽器先執行——命中錢幣時呼叫e.stopImmediatePropagation()擋掉
   receiver那份，錢幣拖曳才不會跟商品拖曳互搶同一次點擊，跟PriceTagSystem
   的做法是同一個道理(見initShadowPopup()的說明)。 */
function attachKvCoinPointerEvents(canvas, redraw){
  ...
  /* 兩個錢幣可能疊在一起，由後面畫的(coin2)蓋在上面，檢查順序反過來
     (coin2優先判定)，比較符合「點到看得到的那個」的直覺。 */
  function hitTest(p){
    var hs = 22;
    for(var i=KV_COIN_KEYS.length-1; i>=0; i--){
      var coinKey = KV_COIN_KEYS[i];
      var box = _kvCoinBox(coinKey);
      if(!box) continue;
  ...
  canvas.addEventListener('pointerdown', function(e){
    ...
    var hit = hitTest(p);
    if(!hit){
      var had = KV_COIN_KEYS.some(function(k){ return _kvCoinSelected[k]; });
      if(had){ KV_COIN_KEYS.forEach(function(k){ _kvCoinSelected[k]=false; }); redraw(); }
      return;
    }
    e.preventDefault();
    e.stopImmediatePropagation(); // 命中錢幣就完全擋掉receiver那份，避免同時觸發商品拖曳
    KV_COIN_KEYS.forEach(function(k){ _kvCoinSelected[k] = (k===hit.coinKey); });
    var cfg = _kvCoinCfg(hit.coinKey);
    _kvCoinInteraction = {
      coinKey: hit.coinKey,
  ...
  canvas.addEventListener('pointermove', function(e){
    var it = _kvCoinInteraction;
    if(!it) return;
    e.preventDefault();
    var p = toCanvasPos(e);
    var cfg = _kvCoinCfg(it.coinKey);
    if(it.mode === 'move'){
      cfg.x = it.startCfg.x + (p.x - it.startPointer.x);
      cfg.y = it.startCfg.y + (p.y - it.startPointer.y);
    } else {
      /* 縮放：用滑鼠到中心的距離比例，等比放大/縮小，4個角效果一致，
         跟SKBN小標的resize邏輯同一套做法。 */
  ...
  ['pointerup','pointercancel'].forEach(function(evt){
    canvas.addEventListener(evt, function(){ _kvCoinInteraction = null; });
  });
}
...
function initShadowPopup(){
  ...
  /* 小標系統：一定要在receiver.attachPointerEvents(...)之前綁，同一個canvas上
     先註冊的監聽器先執行，命中小標時會stopImmediatePropagation()擋掉receiver
     那份，兩套拖曳(商品 vs 小標)才不會互搶同一次點擊，見pricetag-interaction.js
     裡attachPointerEvents的說明。 */
  if(window.PriceTagSystem){
    PriceTagSystem.init(drawShadowCanvas);
    PriceTagSystem.attachPointerEvents(_shadowCanvas, _shadowReceiver, drawShadowCanvas);
  }
  attachKvCoinPointerEvents(_shadowCanvas, drawShadowCanvas);
...
function _loadShadowBg(version){
  ...
  var idx = 0;
  var img = new Image();
  /* 2026-09修正：這份背景圖快取是shadow popup(商品1200畫布)跟coupon
     popup(券樣1200畫布，見js/shadow-system/coupon-popup.js)共用的同一份
     _shadowBgCache——但載入完成/確定讀不到時，這裡以前只會呼叫
     drawShadowCanvas()，如果當下開著的其實是coupon popup(不是shadow
     popup)，_shadowCtx是null(shadow popup根本沒開過)，這個redraw完全
     沒有作用，coupon popup會一直卡在載入前的純色墊底畫面，直到使用者
     關掉重開才會看到真正的背景圖(那時候快取早就load完了)。改成兩個
     popup的redraw函式都呼叫一次，各自函式內部本來就有「popup沒開著就
     直接return」的guard，不會互相干擾，也不會因為對方沒開而報錯
     (用typeof檢查，兩支函式在同一支.js檔案裡都一定會定義到，這裡的
     typeof guard主要是避免載入順序萬一調換造成的邊界情況)。 */
  function _redrawOpenPopups(){
    drawShadowCanvas();
    if(typeof drawCouponCanvas === 'function') drawCouponCanvas();
  }
  function tryNext(){
    if(idx >= candidates.length){ entry.status = 'missing'; _redrawOpenPopups(); return; }
...
function drawShadowCanvas(){
  ...
  /* 2026-09新增：ShadowPlugin.setAngle()/configureZone()是整個頁面共用的
     全域狀態(見shadow-plugin.js)，不是跟著這個1200畫布獨立的——SKBN單品
     popup背景的自動合成(js/shadow-system/skbn-shadow-popup.js的
     ensureSkbnAutoCompose())也會呼叫同一組全域函式，設定它自己那個
     SKBN實例的角度。如果這個1200畫布open著的時候背景剛好跑了一次SKBN
     自動合成，沒有在這裡重新蓋回S.shadowAngle的話，這個畫布接下來的預覽
     就會被SKBN的角度污染，使用者會看到畫面上選的明明是"右"，陰影卻照著
     SKBN那邊剛好在跑的角度變化，兩邊看起來像在互相干擾。每次重畫都在
  ...
  if(window.PriceTagSystem) PriceTagSystem.drawAll(_shadowCtx, _shadowReceiver); // 小標疊在選取框之上，方便拖曳時看得到/點得到
  drawKvCoins(_shadowCtx);
  drawKvCoinSelection(_shadowCtx);
  ...
  displayOrder.forEach(function(slotId, displayIdx){
    ...
    (function(slotId, def, box){
      ...
      if(def.type==='product' && hasImg){
        ...
        /* 小標(原價/特價)：只有商品類、且已經有圖才顯示，跟拍立得同一個條件——
           人物(主持人)不需要小標。 */
        if(window.PriceTagSystem) PriceTagSystem.renderControls(box, slotId, drawShadowCanvas);
      }
...
function applyShadowSlotDataUrl(slotId, type, dataUrl, ratio){
  ...
  _shadowReceiver.setActiveSlot(slotId, drawShadowCanvas);
  renderSlotBar();
  /* 2026-09新增：商品照片一上傳進這個slot，SKBN單品popup就有素材可以自動
     就定位了(見js/shadow-system/skbn-shadow-popup.js的ensureSkbnAutoCompose())，
     不用等使用者在這個1200畫布popup裡按下「確認並套用到主持人圖層」才
     觸發——這裡的drawShadowCanvas()只會重畫這個獨立的1200畫布，不會連帶
     重畫背後的主畫布區(包含SKBN那幾張)，如果不在這裡額外呼叫一次，SKBN
     畫布要等使用者關掉這個popup（confirm或取消都會呼叫renderAll()）才會
     第一次有機會顯示，體感上就是「要整個確認完商品才會出現在SKBN」。
     人物(type==='person')不影響SKBN，不用觸發。 */
  ...
  var overlay = createOverlay(
    '<div class="popup-panel" style="width:'+(SHADOW_DISPLAY+420)+'px;">'+
      ...
      '<div class="popup-foot">'+
        '<button class="tbtn primary" id="shadow-export-btn">確認並套用到作圖區</button>'+
      '</div>'+
    '</div>'
  );
...
function exportShadowComposite(){
  ...
  /* 跟drawShadowCanvas()同一個理由——ShadowPlugin.setAngle()是全域共用
     狀態，SKBN那邊背景自動合成也會用到，這裡在真正合成前明確蓋回
     S.shadowAngle，確保匯出結果的陰影角度一定是畫面上選的那個按鈕，
     不會被SKBN背景合成剛好插隊蓋掉。 */
  ShadowPlugin.setAngle(S.shadowAngle || 'top');
  ...
  if(window.PriceTagSystem) PriceTagSystem.drawAll(octx, _shadowReceiver); // 小標跟著商品一起烤進最終輸出的PNG，不是另外疊加的圖層
  /* 錢幣只在商品模式烤進去——券樣模式的host是設計師外部做好直接匯入的
     成品，不是靠這個1200畫布合成出來的，這裡的錢幣位置(針對商品模式的
     construct畫面)烤進去對券樣模式沒有意義，見檔頭2026-09定案的說明。 */
  if(S.exposureStyle !== 'coupon') drawKvCoins(octx);
