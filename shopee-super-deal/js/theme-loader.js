...
function setTemplateVersion(v){
  ...
  /* mask顏色不用像shadow那樣轉rgb推給其他plugin——modules/mask-module.js
     每次畫的時候會直接讀window.Theme.mask，這裡只要確保window.Theme指向
     正確的版本就好，不用額外同步動作。 */
}

function loadTheme(cb){
  /* 2026-09新增：跟core.js的CONFIG_CACHE_BUST同樣的理由——這個fetch是
