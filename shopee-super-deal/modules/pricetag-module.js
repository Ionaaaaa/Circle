...
window.Modules.pricetag = (function(){
  ...
  /* 2026-09修正：原本的regex /^(\D*)(\d+)(.*)$/ 遇到千分位逗號就會提早
     停止——例如"$1,500"，\d+只會抓到"1"，逗號後面的",500"全部被當成
     suffix(小字級)，變成「1」正常大小、「,500」被誤判成要縮小，使用者
     反映的就是這個現象。改成(\d(?:[\d,]*\d)?)：允許中間夾逗號，但一定
     要以數字開頭、數字結尾(避免不小心吃到句尾多餘的逗號)，這樣"1,500"
     會被當成完整一組digits，不會再被逗號打斷。"$490/32入"這種原本就
     需要吃到"/"為止就停的案例不受影響("/"不是數字也不是逗號，不會被
     這個新規則多吃進去)。 */
  function splitPriceText(str){
    var m = /^(\D*)(\d(?:[\d,]*\d)?)(.*)$/.exec(str || '');
